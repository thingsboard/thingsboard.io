...
| `mappings[].value` | Value to write to the OID; may reference `${params.<name>}` to use a named parameter from the RPC call |
...
<Aside type="note">
  `${params.<name>}` resolves to a named parameter from the incoming RPC call body. For example, calling `dataToMultiSet` with `{"temperature": 22, "pressure": 1013}` makes `${params.temperature}` resolve to `22` and `${params.pressure}` resolve to `1013`, letting one `multiset` request write several independent values from a single RPC call.
</Aside>
