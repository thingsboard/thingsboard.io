...
| `type` | SNMP data type for value encoding — see [Supported data types](#supported-data-types), **required only for `set` method** |
...
<Aside type="note">
  Data type specification via the `type` field is **required only for `set` method** operations in `serverSideRpcRequests`.
</Aside>
