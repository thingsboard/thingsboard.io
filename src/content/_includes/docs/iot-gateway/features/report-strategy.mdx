...
<ImageGallery images={[{ src: '/src/assets/schemas/gateway-report-strategy.svg', alt: 'Report strategy override hierarchy: Default → General (tb_gateway.json) → Connector → Device → Datapoint, each level overriding the one above it.', caption: 'Report Strategy is resolved top-down. Each level overrides the one above it, with the Datapoint-level setting taking the highest priority.' }]} />
