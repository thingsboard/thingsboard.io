---
type: tutorial
title: 'PE Step 4. Share data with Customer'
description: How to get started with ThingsBoard.
---
import { Aside } from '@astrojs/starlight/components';
import DocLink from '@components/DocLink.astro';
import Checklist from '~/components/Checklist.astro';
import Box from '~/components/tutorial/Box.astro';
import { Steps } from '@astrojs/starlight/components';

ThingsBoard supports multi-tenancy, allowing you to securely share devices and dashboards with different customers while keeping their data fully isolated.

<DocLink product={props.product} path='user-guide/TBD/'>Customer</DocLink> in ThingsBoard is a logical container used to isolate data and provide controlled access for external users under a tenant.  

There are multiple ways to grant a Customer access to a specific entity:
- **assign** the Customer as the **owner** of the entity (exclusive access);
- **share** the entity with the Customer (shared access for multiple customers);

In this step, you will:
- create a Customer;
- assign the Customer as the owner of a device;
- share a dashboard with the Customer;

This demonstrates how ThingsBoard provides flexible access management and secure data isolation between customers.

---

## Step 4.1 Create a Customer

To create a new customer:

<Steps>
  1. Navigate to the **Customers**.

  2. Click the **&#43; Add customer**.

  3. Enter **My New Customer** as the title.  
  Optionally, provide an address, phone number, or other additional details.

  4. Click **Add**.
</Steps>

The Customer has been created.

---

## Step 4.2 Change owner of the device

Make the customer the sole owner of the device so that only this customer has access to the device data.

<Steps>
  1. Go to the **Devices**
  
  2. Click on the **Thermostat** device to open its details.

  2. Click **Manage owner and groups**.
  
  3. In the **Owner** line select the your customer from the list.
  
  4. Optionally, create new entity group and add device o them.

  5. Click **Update**.
</Steps>

The device is now owned by the customer.

<Aside type="note">
  You can always transfer the device back to the tenant.
</Aside>

#### Verify the result

Make sure the device is assigned to your customer:
- Go to the **Customers**. 
- Find your customer and click the **Manage customer devices**.
- Verify that the device appears in the customer’s device list.

---

## Step 4.3 Share the dashboard

Now share the dashboard with the customer.

<Aside type="note">
  In ThingsBoard, you cannot share an individual dashboard directly—it must be shared as part of a **dashboard group** that contains the required dashboard.
</Aside>

By default, your dashboard is located in the **All** group. Although the recommended approach is to create a dedicated dashboard group and move your dashboard there, this guide uses the existing **All** group for simplicity.

<Steps>
  1. Navigate to the **Dashboards &#8702; Groups**.
  
  2. Click the **Share** icon next to the **All** dashboard group.

  3. Select the customer with whom you want to share the dashboard and set the permission level. In this case, choose **Read**, then click **Share**.

</Steps>

The customer now has **read-only** access to the dashboard.

#### Verify the result

To confirm that the group is shared with the customer:
- Click the **pencil** icon next to the **All** group to open its details.
- Open the **Permissions** tab to view all sharing permissions for this group.

---

<Box icon="check-list">

  ### Checklist for moving on

  <Checklist>
    - [ ] The Customer has been granted all required permissions.
  </Checklist>
</Box>
