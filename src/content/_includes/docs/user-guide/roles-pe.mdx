...
<ImageGallery images={[{ src: '/src/assets/schemas/generic-role.svg', alt: 'Generic role scope: Bob (Tenant Admins) sees Device A1 and Device B1; Alice (Customer B Admins) only sees Device B1.', caption: 'The same Generic role behaves differently depending on assignment level. At Tenant level it grants tenant-wide access; at Customer level it is limited to that customer and its sub-customers.' }]} />
