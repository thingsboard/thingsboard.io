...
<ConditionalHeading level={3} id="extension-limits" showFor="pe">Extension limits</ConditionalHeading>

Defines how many <DocLink product={props.product} path="reference/extensions/overview">extensions</DocLink> a tenant can create, and how much compute they may use in total:
...
<ImageGallery images={[{ src: '/src/assets/images/reference/extensions/tenant-profile-extension-limits.png', alt: 'The Extensions group in the tenant profile details, with the three limit fields: extensions maximum number, total CPU limit in cores, and total memory limit in megabytes', caption: 'The Extensions group of the tenant profile. Only a system administrator opens this form; tenants see the resulting quota on the Extensions page.' }]} />
...
- Max data points number in rolling arguments — caps the per-argument **Max values** field; see <DocLink product={props.product} path="user-guide/calculated-fields/#limits--quotas" target="_blank">Calculated Fields — Limits & Quotas</DocLink> for how the effective window can end up shorter than configured.
