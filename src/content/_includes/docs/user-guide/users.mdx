...
<br/>{props.product !== Products.CE && <>ThingsBoard uses a <DocLink product={props.product} path='user-guide/roles/' target="_blank">Role-Based Access Control (RBAC)</DocLink> model:
  <ul>
...
<strong>User groups</strong> aggregate users with similar access requirements and simplify bulk permission management. A user can belong to multiple groups simultaneously and inherits permissions from all assigned groups.<br/>
  <br/>ThingsBoard provides built-in user groups and supports creation of custom groups:
<ul>
