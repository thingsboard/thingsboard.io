...
A solution can also ship its own backend <DocLink product={props.product} path='reference/extensions/overview'>extension</DocLink> at `solutions/<slug>/extension/`, next to a project-wide one at the project root. All `tb extension` commands accept the same `--solution` flag — see <DocLink product={props.product} path='reference/extensions/cli-deployment#root-and-solution-extensions'>Root and solution extensions</DocLink>.
