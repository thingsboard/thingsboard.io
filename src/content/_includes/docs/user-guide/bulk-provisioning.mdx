...
A common use case is pre-loading a batch of devices before shipping so they can use <DocLink product={props.product} path='user-guide/device-provisioning'>Device provisioning</DocLink> with the "Check Pre-Provisioned Devices" strategy: the devices are known to ThingsBoard in advance and only those exact devices are allowed to self-register.
