...
<DocLink product={props.product === Products.CE ? Products.PE : props.product} path="recipes/reporting-subreport-daily-alarms">Daily Alarm Report per Device Using Subreports</DocLink> — step-by-step walkthrough: create a subreport template and a master report that generates one alarm section per device in a single PDF.
