...
This is distinct from <DocLink product={props.product} path='user-guide/claiming'>Claiming</DocLink>: provisioning creates the device and gives it credentials; claiming transfers an already-provisioned device to a specific customer.
...
<Aside type="tip">
  See **CSV template and examples** for a ready-to-use file. For automated pipelines (CI/CD, ERP, manufacturing systems) the same import is available via the <DocLink product={props.product} path='reference/rest-api'>Bulk Import REST API</DocLink>.
</Aside>
...
<DocLink product={props.product} path='user-guide/device-profiles'>Open the device profile</DocLink>, go to the **Device Provisioning** tab, select a strategy, and copy the generated **Provision device key** and **Provision device secret** — these go into the device firmware.
