...
{(props.product === Products.CE || props.product === Products.PE) && (
  <>
    ...
    <p>Your AI credit allowance comes with your <strong>license</strong> and applies to the whole instance — all tenants on it draw from the same pool. Your system administrator can set an additional per-tenant cap in the <DocLink product={props.product} path="user-guide/tenant-profiles/" target="_blank">Tenant Profile</DocLink>.</p>
    {props.product === Products.PE && (
      <p>ThingsBoard Private Cloud customers: the AI credit allowance included with each plan is published on the <a href="/docs/private-cloud/subscription/#ai-credits" target="_blank" rel="noopener noreferrer">Private Cloud subscription page</a>.</p>
    )}
    <ImageGallery images={[
      {
