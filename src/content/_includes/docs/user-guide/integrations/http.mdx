...
HTTP Integration is an entry point for data from any HTTP-capable source — devices, cloud services, custom backends, or third-party services — via standard HTTP `POST` requests. Use it to bring data from any system that can issue an HTTP request into ThingsBoard for monitoring and rule-based automation — including returning commands as the response to a device's next request.
...
<ImageGallery images={[
  { src: '/src/assets/images/user-guide/integrations/http-integration-architecture.svg', alt: 'HTTP integration architecture diagram showing a device or service sending an HTTP POST request to the ThingsBoard HTTP endpoint, which the uplink converter decodes into telemetry and attributes stored by the ThingsBoard Core Service, with a queued downlink payload returned in the HTTP response to the same request', caption: 'HTTP integration architecture — uplink and downlink data flow between the HTTP source and ThingsBoard.' },
]} />

A device or service sends an HTTP POST request to the ThingsBoard HTTP endpoint. The uplink converter decodes the payload into telemetry and attributes, and the ThingsBoard Core Service stores the data, creating the device automatically on first contact when **Allow create devices or assets** is enabled. Downlink is not pushed separately: when a downlink is queued, the downlink converter encodes the Rule Engine message, and ThingsBoard returns it as the response body to the device's next uplink request.
...
To set up the **HTTP** integration, first create an uplink data converter to process incoming messages, then create and configure the integration.
...
<Steps>
  <ol start="1">
    ...
    <li>
      ...
      <ul>
        <li>Click <strong>Select existing</strong> and choose the <a href="#create-the-uplink-data-converter"><strong>HTTP Uplink Converter</strong></a> created in the previous step.</li>
...
<Steps>
  ...
  4. In the **Add data converter** dialog:
      - Enter a name for the converter, for example, `HTTP Downlink Converter`.
...
var result = {
    // Specify the HTTP response body format: JSON, TEXT, or BINARY.
    contentType: "JSON",

    // Serialize the incoming message and use it as the HTTP response body.
