...
<Steps>
  <ol start="3">
    ...
    <li><strong>Name</strong> — enter a name, for example <code>HTTP Uplink Converter</code>.</li>
    <li><strong>Main decoding configuration</strong> — a code editor with the function signature <code>function decoder(payload, metadata) &#123;</code>.<br />By default it opens in TBEL; use the <strong>TBEL / JS</strong> toggle (upper right) to switch languages.</li>
  </ol>
</Steps>
...
<h4>Basic converter template</h4>

A full working template for JSON payloads — adapt field names to match your device.
...
Use the built-in <DocLink product={props.product} path="user-guide/integrations/uplink-data-converter/#test-payload-decoder" target="_blank">**payload decoder testing tool**</DocLink>.  
...
After a successful test, click **Save** to save the converter.
...
<Steps>
  <ol start="1">
    ...
    <li>
      ...
      <ul>
        <li>Click <strong>Select existing</strong> and choose the <a href="#create-http-uplink-data-converter">previously created</a> uplink converter from the dropdown, then click <strong>Next</strong>.</li>
      </ul>
      <Aside type="note">
...
| `401 Unauthorized` | Invalid Basic Auth credentials | Check login/password or disable auth |
