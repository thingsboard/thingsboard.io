...
- You have at least one LoRaWAN device registered in ThingPark, with its **DevEUI** and **AppEUI** on hand — see **[ThingPark Setup](#thingpark-setup)** below if you don't have one yet.

## Create ThingPark Uplink Data Converter

The uplink converter decodes the incoming ThingPark message and maps it to the ThingsBoard data model — it resolves the target device and extracts fields into telemetry and attributes. For the full decoder function reference, see <DocLink product={props.product} path="user-guide/integrations/uplink-data-converter" bold={true} target="_blank">Uplink data converter</DocLink>. 
...
<Steps>
  <ol>
    ...
    <li>
      ...
      <ul>
        <li>Click <strong>Skip</strong> — a downlink converter is only needed when ThingsBoard must send commands back to devices. It can be added later.</li>
      </ul>
    </li>
    <li>
...
<Steps>
  ...
  5. Configure the node:
      - **Name** — `Downlink to ThingPark`.
      - **Integration** — select your **ThingPark integration**.
      - Click **Add**.
  6. Connect the **Message Type Switch** node to the **Fetch eui and fPort** node via the **Post attributes / Attributes Updated** relations.
  7. Connect the **Fetch eui and fPort** node to the **Downlink to ThingPark** node via the **Success** relation.
