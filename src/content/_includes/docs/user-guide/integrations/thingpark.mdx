...
To set up the **ThingPark** integration, first create an uplink data converter to process incoming messages, then create and configure the integration.
...
### Create the ThingPark Integration
...
<Steps>
  <ol>
    ...
    <li>
      ...
      <ul>
        <li>Click <strong>Select existing</strong> and choose the <a href="#create-the-uplink-data-converter"><strong>ThingPark Uplink Converter</strong></a> created above, or click <strong>Create new</strong> to define the decoder inline.</li>
        <li>Click <strong>Next</strong>.</li>
      </ul>
    </li>
    <li>
...
- **Downlink URL** — pre-filled with ThingPark's downlink delivery endpoint (e.g. `https://api.thingpark.com/thingpark/lrc/rest/downlink`). Required only if you configure a downlink converter and rule chain — see <a href="#configure-downlink"><strong>Configure and Test Downlink</strong></a>.
...
Configure your ThingPark application to route uplink messages to the ThingsBoard **HTTP endpoint URL** copied in <a href="#create-the-thingpark-integration"><strong>Create the ThingPark Integration</strong></a>. 
...
Replace `$HTTP_ENDPOINT_URL` with the **HTTP endpoint URL** copied in **[Create the ThingPark Integration](#create-the-thingpark-integration)**:
...
<Steps>
  1. Go to **Integrations center &#8702; Integrations** and open the **ThingPark integration**.
  ...
  4. In the **Add data converter** dialog:
      - Enter a name for the converter, for example, `ThingPark Downlink Converter`.
...
<h4>Check Delivery on ThingPark</h4>
