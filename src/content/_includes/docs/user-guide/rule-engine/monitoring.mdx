...
<ImageGallery images={[{ src: '/src/assets/schemas/view-debug-events.svg', alt: 'View debug events', caption: 'Typical debug trace for a failing script node: an IN event shows the incoming message, and a corresponding OUT event shows the Failure relation with the error details.' }]} />
