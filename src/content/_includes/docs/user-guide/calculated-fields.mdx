...
- **Entity level** — applied to a single <DocLink product={props.product} path="user-guide/devices" bold={true} target="_blank">Device</DocLink> or <DocLink product={props.product} path="user-guide/assets" bold={true} target="_blank">Asset</DocLink>.
- **Profile level** — defined in a <DocLink product={props.product} path="user-guide/device-profiles" bold={true} target="_blank">Device profile</DocLink> or <DocLink product={props.product} path="user-guide/asset-profiles" bold={true} target="_blank">Asset profile</DocLink> and inherited by every entity using that profile.
...
## Configure Calculated Fields with AI
...
<Steps>
1. On the **Calculated fields** page, click **Configure with AI** to open the AI Assistant panel.
...
| Data points per rolling argument | 1000 | Maximum number of `{ ts, value }` records in a time series rolling window. This argument always fetches raw (unaggregated) values — if the window holds more points than this limit, the oldest points are silently dropped, so the effective window can be shorter than configured. || State size | 32 KB | Maximum total state stored per calculated field (geofencing zone state, rolling window cache, etc.) |
