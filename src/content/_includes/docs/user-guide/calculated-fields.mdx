...
| Data points per rolling argument (tenant limit) | 1000 | Upper bound on the **Limit** field of a "Time series rolling" argument. The argument's own **Limit** field — pre-filled at 1/10 of this value (100 by default) — is the effective cap: it always fetches raw (unaggregated) values, and once the window holds more points than that field's value, the oldest points are silently dropped, so the effective window can be shorter than configured. |
