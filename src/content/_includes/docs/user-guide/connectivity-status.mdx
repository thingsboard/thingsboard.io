...
The Device State service fires four event types into the Rule Engine. You can configure reactions to each using custom <DocLink product={props.product} path='user-guide/rule-engine'>rule chains</DocLink>.

| Event | When it fires |
|-------|--------------|
| <DocLink product={props.product} path='reference/rule-engine/message-types'>**Connect**</DocLink> | Device establishes a session (MQTT) or sends any request (HTTP/CoAP) |
| <DocLink product={props.product} path='reference/rule-engine/message-types'>**Disconnect**</DocLink> | Session closes (MQTT) or request completes (HTTP/CoAP) |
| <DocLink product={props.product} path='reference/rule-engine/message-types'>**Activity**</DocLink> | An inactive device sends telemetry, updates attributes, or sends an RPC command |
| <DocLink product={props.product} path='reference/rule-engine/message-types'>**Inactivity**</DocLink> | No activity received within `DEFAULT_INACTIVITY_TIMEOUT` seconds |
