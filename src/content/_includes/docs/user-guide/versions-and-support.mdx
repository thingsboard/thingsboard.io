...
- **Deprecation policy:** Features flagged as deprecated in a Minor release will remain for at least one subsequent Minor release before removal (or be guarded by feature flags when possible). Deprecations and removals are always called out in <DocLink product={props.product} path="user-guide/releases-table">release notes</DocLink>.
