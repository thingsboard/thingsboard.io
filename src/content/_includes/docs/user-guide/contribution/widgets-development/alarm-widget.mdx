...
<Aside type="caution">
  Alarm widgets require `<tb-alarms-table-widget style="display: none;" [ctx]="ctx">` to be present in the HTML — even if hidden. ThingsBoard only initializes the alarm subscription when this component exists in the DOM. Without it, `self.ctx.defaultSubscription.alarms` will be `undefined` and `onDataUpdated` will never receive alarm data.
</Aside>
...
2. In the **HTML** tab, paste:
   ...
   ```html
   <!--
     Required: tb-alarms-table-widget must be initialized for the alarm
     subscription to work. It is hidden visually but must be present in
     the DOM — without it, self.ctx.defaultSubscription.alarms will be
     undefined and no alarm data will arrive.
   -->
...
4. In the **JavaScript** tab, paste:
   ...
   **Alarm data access** — alarms arrive at `self.ctx.defaultSubscription.alarms.data`, a `PageData<AlarmInfo>` object where `.data` is the flat array of alarm objects. The platform handles filtering by severity, status, and type based on the widget's alarm filter configuration.
...
2. In the **HTML** tab, paste:
   ...
   ```html
   <!--
     Required: initializes the alarm subscription.
     Hidden but must be present in the DOM.
   -->
   <tb-alarms-table-widget
     style="display: none;"
     [ctx]="ctx">
