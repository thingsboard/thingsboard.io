...
Assign a dedicated <DocLink product={props.product} path='user-guide/rule-engine'>rule chain</DocLink> to an asset profile to isolate processing logic — attribute updates and lifecycle events (Created/Updated/Deleted) — for that asset type without overloading the Root Rule Chain.
...
Define conditions that apply to all assets in this profile — for example, trigger a Critical alarm when a computed value exceeds a threshold. Rules defined here are shared across every asset using the profile. See <DocLink product={props.product} path='user-guide/alarm-rules'>Alarm Rules</DocLink>.
