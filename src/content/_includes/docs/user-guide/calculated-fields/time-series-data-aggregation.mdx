...
<ImageGallery images={[
  { src: '/src/assets/schemas/aggregation-interval-offset.svg', alt: 'Apply offset diagram', caption: 'With a +10 min offset, interval boundaries shift from 00:00/01:00 to 00:10/01:10. Telemetry arriving just after the hour is included in the correct interval.' },
  { src: '/src/assets/schemas/aggregation-interval-await.svg', alt: 'Await timeout diagram', caption: 'After the interval ends, the system waits for the configured timeout before producing the final result. Late data that arrives within the window is included. Data arriving after the timeout triggers a recalculation.' },
  { src: '/src/assets/schemas/aggregation-interval-intermediate.svg', alt: 'Intermediate results diagram', caption: 'During an open interval, partial aggregates are computed every 5 minutes. Dashboards can display the latest intermediate result as "current hour average so far".' },
]} />
