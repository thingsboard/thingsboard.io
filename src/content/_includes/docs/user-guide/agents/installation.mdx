...
- The agent replaces its own container on command from ThingsBoard, so a new agent release is rolled out from the UI. See <DocLink product={props.product} path='user-guide/agents/self-upgrade'>Agent self-upgrade</DocLink>.
