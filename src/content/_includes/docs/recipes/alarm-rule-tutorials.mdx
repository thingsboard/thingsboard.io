...
These tutorials walk through common alarm rule configurations using a Thermometer device that sends `temperature` telemetry. Each example builds on the previous one. For the full configuration reference, see <DocLink product={props.product} path="user-guide/alarm-rules">Alarm rules</DocLink>.
...
Download the setup file and <DocLink product={props.product} path="user-guide/bulk-provisioning">import</DocLink> it before running the examples:
...
<Steps>
1. Go to **Alarms → Alarm rules** and click **+**, then select **Create new alarm rule**.
...
**Continuation of [Example 1](#example-1-simple-condition-temperature-threshold).**
...
**Preparation:** Add a <DocLink product={props.product} path="user-guide/digital-twins/attributes/#server-side-attributes">server attribute</DocLink> to *Thermometer*:
