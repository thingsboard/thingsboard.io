...
The solution automatically configures the IoT Gateway and creates five drilling devices via the <DocLink product={props.product} path='iot-gateway/what-is-iot-gateway'>ThingsBoard IoT Gateway</DocLink> with the <DocLink product={props.product} path='iot-gateway/config/modbus'>Modbus connector</DocLink>:
...
Alarm generation is handled by <DocLink product={props.product} path='user-guide/alarm-rules'>alarm rules</DocLink> configured in the device profiles for each drilling device. Alarms monitor critical safety thresholds and operational efficiency, including the Blowout Preventer (BOP) for well pressure and flow rate anomalies to ensure fail-safe operation and reduce blowout risk.
