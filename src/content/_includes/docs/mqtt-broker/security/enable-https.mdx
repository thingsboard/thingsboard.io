...
The advantage of this option is a simple configuration.
...
We recommend using valid SSL certificates generated using trusted CA authorities and avoiding spending time on resolving issues with [self-signed certificates](/docs/mqtt-broker/security/mqtts/#self-signed-certificates-generation).
...
To generate self-signed certificates for testing purposes, refer to the [Self-signed certificates generation](/docs/mqtt-broker/security/mqtts/#self-signed-certificates-generation) section in the MQTT over SSL guide.
