...
<ImageGallery images={[
  {
    ...
    caption: 'Select the verification type that matches how your tokens are issued. If unsure, HMAC-based is usually the easiest to start with during testing.',
  },
]} />
...
You need a valid JWT token, or you can generate one with any language that supports HS256 signing. Here's a simple **Python** example.
...
You can verify the token's claims and signature using [jwt.io](https://jwt.io/).
...
- `YOUR_CLIENT_ID` — your client ID. In our example, it should equal `mqtt-client-id` to pass the authentication claim check `sub=${clientId}` in the JWT provider.
