...
<Aside type="caution">
  Enabling this setting weakens the certificate-based security model. It is not recommended for production deployments.
</Aside>
...
<Aside type="note">
  If either **pubAuthRulePatterns** or **subAuthRulePatterns** is set to `null` or an empty list (`[]`), the client will not be able to publish to or subscribe from any topics.
</Aside>
...
<Tabs>
  <TabItem label="X.509 Certificate chain">
    ...
    2. **Generate client certificate chain**
       ...
       We need a certificate chain with reasonable Common Names (CNs). We will use the intermediate certificate to sign certificates for our devices.
       ...
       **Step 2.3: Generate device certificate**
       ...
       In the next steps we will use the device key file `deviceKey.pem` and the certificate chain `chain.pem`.
