...
### Database node group (optional)

Starting with chart version 2.0.0, the TBMQ Helm chart no longer bundles PostgreSQL. Use the `tbmq-postgresql`
node group only if you plan to self-host PostgreSQL inside EKS (for example, via the CrunchyData PGO operator):
...
- You're using Amazon RDS or an existing managed PostgreSQL service.
