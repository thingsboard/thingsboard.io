...
You can also self-host any of them on the dedicated `tbmq-postgresql`, `kafka`, and `redis` node groups defined in
`cluster.yml`. For an RDS-specific provisioning walkthrough, see the
<DocLink product={props.product} path="install/cluster/aws-cluster-setup/#amazon-postgresql-db-configuration">AWS cluster setup guide</DocLink>.
...
```bash
kubectl get pvc -n tbmq -o name | grep tbmq-node-data | xargs -r kubectl delete -n tbmq
```
