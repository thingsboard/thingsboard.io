...
- **Scalability** — horizontally scalable, built on cutting-edge open-source technologies.
- **Fault tolerance** — no single point of failure; every node in the cluster is functionally identical.
- **Robustness and efficiency** — handles millions of clients and processes millions of messages per second.
...
At ThingsBoard, experience building scalable IoT applications has surfaced three primary MQTT messaging scenarios:
...
- [Kafka](https://kafka.apache.org/) as the underlying backbone prevents message loss even during node or client failures.
...
and subscriptions for efficient processing. When a client disconnects from one node, other nodes seamlessly
continue based on the latest state. Newly added nodes receive this state on startup.
...
<DocLink product={props.product} path="reference/architecture/persistent-device-client">Persistent DEVICE client</DocLink> reference.
...
[MQTT 5 shared subscriptions](/docs/mqtt-broker/user-guide/shared-subscriptions/#application-client-type)
...
| `tbmq.sys.historical.data` | Historical statistics (incoming/outgoing message counts, etc.) per broker node |
| `tbmq.client.blocked` | Distributes and stores the blocked clients list across the cluster |
| `tbmq.sys.internode.notifications.$SERVICE_ID` | System notifications between broker nodes for auth provider sync and cache cleanup |

### Redis

[Redis](https://redis.io/) is an in-memory data store used in TBMQ to persist messages for DEVICE persistent
...
TBMQ uses [PostgreSQL](https://www.postgresql.org/) to store users, credentials, MQTT client credentials,
...
TBMQ uses [Netty](https://netty.io/) as the TCP server for the MQTT protocol. Netty's asynchronous,
event-driven model handles large numbers of simultaneous connections efficiently using non-blocking I/O (NIO) —
no dedicated thread per connection is needed. This enables high throughput and low-latency communication under
heavy loads, with built-in support for TLS encryption.

### Actor system

TBMQ uses a custom Actor System to handle MQTT clients concurrently. Two actor types:

- **Client actors** — one per connected MQTT client; handles CONNECT, SUBSCRIBE, UNSUBSCRIBE, PUBLISH, and related messages.
- **Persisted Device actors** — one per persistent DEVICE client; manages persistence operations and message storage/retrieval for offline delivery.

See the [Actor model](https://en.wikipedia.org/wiki/Actor_model) for background.
...
The [Trie](https://en.wikipedia.org/wiki/Trie) data structure provides fast topic matching:
...
All subscriptions are consumed from Kafka and stored in the Trie in memory. When a PUBLISH message arrives, the
broker queries the Trie with the topic name to find all matching subscribers, then delivers a copy to each. The
trade-off is increased memory consumption proportional to the number of active subscriptions.
