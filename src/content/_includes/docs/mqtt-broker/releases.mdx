...
{props.product === Products.TBMQ_PE && (
  <>
    <ConditionalHeading level={2} id="v220pe">v2.2.0 (January 20, 2026)</ConditionalHeading>
    <p>
      This release includes all features, improvements, and bug fixes from{' '}
      <a href="#v220">TBMQ v2.2.0</a>, along with the following Professional Edition exclusive updates:
    </p>

    <p><strong>Main features</strong></p>
    <ul>
      <li>
        <strong>Single Sign-On / OAuth 2.0:</strong> Enable seamless integration with corporate identity
        providers (IdPs) via SSO and the OAuth 2.0 framework. This centralizes user management, enforces
        strong authentication, and improves the overall security posture for TBMQ administrators.
      </li>
      <li>
        <strong>Role-Based Access Control (RBAC):</strong> Introduce a robust RBAC system allowing
        administrators to define fine-grained access permissions based on roles. This ensures users only
      ...
      <li>
        ...
        enterprise users to integrate the broker management UI seamlessly into their own product ecosystem.
      </li>
    </ul>
    ...
    <ul>
      ...
      <li>
        ...
        authentication provider when switching between PEM and JWKS formats, ensuring stable
        authentication across dynamic key rotation setups.
      </li>
    </ul>

    <ConditionalHeading level={2} id="ce-release-history">Community Edition release history</ConditionalHeading>
    <p>The following release notes cover all Community Edition versions included in TBMQ.</p>
  </>
)}

## v2.2.0 (September 1, 2025) \{#v220\}
...
- Core and install scripts:
  ...
  - [#169](https://github.com/thingsboard/tbmq/pull/169) Latest version available logic moved from frontend to backend, works in a scheduled fashion to prevent rate limiting;
...
- Core:
  - Performance improvements for message processing. See commits
