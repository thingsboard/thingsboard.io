...
{props.product !== Products.TBMQ_PE && (
  <>
    <p>TBMQ supports both cluster-wide rate limits and per-client publish rate limits:</p>
...
{props.product === Products.TBMQ_PE && (
  <>
    <p>Cluster-wide throughput is enforced by the TBMQ subscription plan — the <strong>Throughput (msg/sec)</strong> tier you purchase caps the total incoming message rate across the cluster, so no separate configuration is needed. Per-client publish rate limits remain configurable:</p>
