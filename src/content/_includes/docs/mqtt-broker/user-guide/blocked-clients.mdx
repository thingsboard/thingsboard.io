...
Blocked client entries are stored in memory for fast matching and synchronized across all broker nodes in a cluster using Kafka,
...
<Aside type="caution">
  Blocked client checks are performed **before** the authentication phase.
...
{props.product === Products.TBMQ_PE && (
  <ImageGallery images={[
    {
      src: '/src/assets/images/docs/mqtt-broker/pe/other/blocked-clients.png',
      alt: 'Blocked clients management page in TBMQ PE',
    },
  ]} />
)}
...
The `REGEX` type matches against one of: `BY_CLIENT_ID`, `BY_USERNAME`, or `BY_IP_ADDRESS`.
