...
- **Topic names** — used by publishers to send messages. Must not contain wildcards or start with `$` (reserved for system topics such as `$SYS`).
...
- **Avoid subscribing to `#`** — receiving every message from all topics can overwhelm a client and cause performance issues.
...
- `{TopicFilter}` — the topic filter, which supports wildcards.
