...
<ImageGallery product={props.product} images={[
  {
    src: '/src/assets/schemas/mqtt-broker-overview.svg',
    alt: 'MQTT broker architecture: four publishers send PUBLISH packets to the broker; the broker routes messages by topic to four subscribers via Subscribe / Publish.',
    caption: 'Publishers send messages to the broker; the broker matches each message against active subscriptions and forwards it to interested subscribers.',
  },
]} />
