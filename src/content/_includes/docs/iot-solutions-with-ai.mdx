...
This section covers the AI tools available in ThingsBoard — from generating new solutions, to running anomaly detection on live telemetry, to opening the platform up to external AI agents. AI for IoT in ThingsBoard works at every stage of a project. Each tool is documented separately below.
...
<CardGrid>
  ...
  <Card title="AI Solution Creator" icon="puzzle" headingLevel={3}>
    ...
    Designed for **creating new solutions from scratch directly inside the ThingsBoard UI**. Describe your use case in plain English — _"Track temperature in cold storage facilities and alert on spoilage"_ — and it generates entity profiles, alarm rules, calculated fields, roles, and dashboards, then deploys everything straight to your tenant. Works with new configurations only — to edit existing dashboards, alarm rules, or calculated fields, use the AI Assistant.
