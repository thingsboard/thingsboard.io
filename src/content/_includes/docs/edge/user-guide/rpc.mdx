...
```plaintext
  ...
  Edge / Server ──── RPC command ──────▶ Device
  Edge / Server ◀─── response (optional) ─ Device


  Client-side RPC (device → platform)

  Device ──── request ──────▶ Edge ──── processed by rule chain
  Device ◀─── response ──────── Edge
```
...
```plaintext
  incoming message
       ...
       ▼
  ┌──────────────┐    Success    ┌──────────────────┐
  │ script node  │──────────────▶│ RPC call request │──▶ device
  │ (build params)│              │  (send command)  │
  └──────────────┘              └──────────────────┘
```
