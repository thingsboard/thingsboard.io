...
mosquitto_pub -h "$EDGE_HOST" -t "v1/gateway/attributes" \
  -u "$ACCESS_TOKEN" \
  -m '{"Device A": {"fw_version": "1.0.3", "battery": 87}}'
```

### Request attribute values

Subscribe to the response topic first, then publish the request. Both have to happen in the same MQTT session, so use [`mosquitto_rr`](https://mosquitto.org/man/mosquitto_rr-1.html): it subscribes, publishes the request and waits for the response over one connection.
...
```json
{
  "id": 1,
  "device": "Device A",
  "client": {"fw_version": "1.0.3", "battery": 87},
  "shared": {"targetFwVersion": "2.0.1"}
}
```

The response is scope-separated: `client` contains client-side attributes, `shared` contains shared attributes. A scope appears only if it returned at least one attribute, so it is absent both when you did not ask for it and when the device has nothing in it.

**Example:**

```bash
mosquitto_rr -d -q 1 -h "$EDGE_HOST" -p 1883 \
  -t "v1/gateway/attributes/request" \
  -e "v1/gateway/attributes/response" \
  -u "$ACCESS_TOKEN" \
  -m '{"id":1,"device":"Device A","clientKeys":"fw_version,battery","sharedKeys":"targetFwVersion"}'
```

To retrieve every attribute of one scope, pass an empty string — `'{"id":1,"device":"Device A","sharedKeys":""}'` returns all shared attributes.
