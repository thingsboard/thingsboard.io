...
<Aside type="note">
  `ADDED_TO_ENTITY_GROUP` and `REMOVED_FROM_ENTITY_GROUP` are available in <ProductBadge />.
</Aside>
...
<DocLink product={props.product === Products.EDGE_PE ? Products.EDGE_PE : Products.EDGE} path="reference/rule-engine/nodes/filter/message-type-switch">Message Type Switch</DocLink> or
<DocLink product={props.product === Products.EDGE_PE ? Products.EDGE_PE : Products.EDGE} path="reference/rule-engine/nodes/filter/message-type-filter">Message Type Filter</DocLink>
