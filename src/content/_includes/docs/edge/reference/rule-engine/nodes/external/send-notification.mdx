...
Use this node to dispatch notifications to users via the <DocLink product={Products.CE} path="user-guide/notifications">notification center</DocLink> — for example, sending a web notification to on-call engineers when a critical alarm is created, or emailing a tenant administrator when a device goes offline.
