...
Use this node to trigger <DocLink product={Products.CE} path="user-guide/calculated-fields">calculated field</DocLink> evaluation against incoming telemetry or attribute data **without persisting that data to the database** — for example, computing a derived metric such as a heat index from incoming temperature and humidity values and publishing the result to the rule chain, while keeping the raw values out of the time-series storage.
