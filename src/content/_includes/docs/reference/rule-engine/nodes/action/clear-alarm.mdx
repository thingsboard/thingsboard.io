...
- **Alarm details script** — a <DocLink product={props.product} path="user-guide/tbel">TBEL</DocLink> or JavaScript function that generates the content for the alarm's `details` field when clearing. The function must return a valid JSON value (primitive, array, or object) and has access to:
  - `msg` — message data
