...
The most flexible transformation node in the rule engine. Write any <DocLink product={props.product} path="user-guide/tbel">TBEL</DocLink> or JavaScript function to reshape the message payload, change its type, rewrite metadata, or split one incoming message into multiple outgoing ones (fan-out). If the pre-built transformation nodes don't cover your use case, this one will.
