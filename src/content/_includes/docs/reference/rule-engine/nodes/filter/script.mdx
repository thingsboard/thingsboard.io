...
Use this node to evaluate a custom boolean condition against an incoming message. It is the most flexible filter in the rule engine: you write a <DocLink product={props.product} path="user-guide/tbel">TBEL</DocLink> or JavaScript function that inspects the message data, metadata, and type, then returns `true` or `false` to route the message accordingly.
