...
<Aside type="note" title="X.509: telling code 2 and code 5 apart">
  ...
  - **Code 5 (Not authorized)** means a certificate *was* received, but ThingsBoard couldn't find a matching device. How that match happens depends on which X.509 method you're using: with the **Individual Certificate** method, matching is based entirely on the certificate's content, never on its Common Name or the MQTT Client ID — ThingsBoard computes a SHA3-256 hash of the certificate (markers and line breaks stripped) and looks up a device credential with that exact fingerprint. With the **Certificate Chain** method, the device is instead identified by extracting its name from the certificate's Common Name via the configured regex (see [Certificate Chain](#certificate-chain-recommended-for-fleets) above) — so a CN/regex mismatch there is a valid thing to check.
