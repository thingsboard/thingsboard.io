...
```bash
mosquitto_pub -d -q 1 -h "$THINGSBOARD_HOST" -p 1883 -t "v2/t" -u "$ACCESS_TOKEN" -m '{"temperature": 25}'
```
...
```bash
mosquitto_pub -d -q 1 --cafile ca-root.pem -h "$THINGSBOARD_HOST" -p 8883 -t "v2/t" -u "$ACCESS_TOKEN" -m '{"temperature": 25}'
```
...
```bash
mosquitto_pub -d -q 1 -h "$THINGSBOARD_HOST" -p 1883 -t "v2/t" -i "$CLIENT_ID" -u "$USERNAME" -P "$PASSWORD" -m '{"temperature": 25}'
```

**MQTT over TLS (port 8883) — one-way SSL:**

```bash
mosquitto_pub -d -q 1 --cafile ca-root.pem -h "$THINGSBOARD_HOST" -p 8883 -t "v2/t" -i "$CLIENT_ID" -u "$USERNAME" -P "$PASSWORD" -m '{"temperature": 25}'
```
...
3. Connect using the certificate and key:
   ```bash
   mosquitto_pub --cafile ca-root.pem -d -q 1 -h "$THINGSBOARD_HOST" -p 8883 -t "v2/t" --key key.pem --cert cert.pem -m '{"temperature": 25}'
   ```
</Steps>
...
3. Connect using the chain and device key:
   ```bash
   mosquitto_pub --cafile ca-root.pem -d -q 1 -h "$THINGSBOARD_HOST" -p 8883 -t "v2/t" --key deviceKey.pem --cert chain.pem -m '{"temperature": 25}'
   ```
</Steps>
