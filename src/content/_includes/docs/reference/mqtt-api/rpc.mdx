...
The following example is written in JavaScript and is based on [mqtt.js](https://github.com/mqttjs/MQTT.js). A pure command-line example is not available for server-side RPC: the device has to publish its response to a topic derived from the incoming request, which the command-line MQTT clients cannot do.
