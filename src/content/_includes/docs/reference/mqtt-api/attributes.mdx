...
```json
{"client": {"firmwareVersion": "2.1.0", "serialNumber": "SN-4A21F"}, "shared": {"targetTemperature": 24, "enabled": true}}
```

</TabItem>
<TabItem label="All client-side attributes">
...
The response includes a scope only if that scope returned at least one attribute.

**Example**

The following example is written in JavaScript and is based on [mqtt.js](https://github.com/mqttjs/MQTT.js). A pure command-line example is not available: ThingsBoard only accepts a subscription to `v1/devices/me/attributes/response/+`, and the command-line MQTT clients cannot use a wildcard topic to receive a response.
...
1. Save the <a href="/resources/docs/reference/mqtt-api/mqtt-js-attributes-request.js" download="mqtt-js-attributes-request.js">**mqtt-js-attributes-request.js**</a> file to your PC.
   ...
   client.on('connect', function () {
       ...
       // ALL keys in that scope, e.g. sharedKeys: "". Omit a field to exclude that scope.
...
2. Now, follow these steps:
   ...
   Install the mqtt package and run the example:
