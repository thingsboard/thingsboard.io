...
The MQTT Provisioning API lets a device register itself with ThingsBoard on first boot — before it has any credentials. See <DocLink product={props.product} path='user-guide/provisioning'>Provisioning</DocLink> for the full workflow and how to configure the device profile.
...
<HostCode
  ...
  lang="bash"
  code='mosquitto_sub -d -q 1 -h "{MQTT_HOST}" -p 1883 -t "/provision/response" -u "provision"'
/>
...
product={props.product}
lang="bash"
code={`mosquitto_pub -d -q 1 -h "{MQTT_HOST}" -p 1883 -t "/provision/request" -u "provision" -m '{"deviceName": "DEVICE_NAME", "provisionDeviceKey": "PUT_PROVISION_KEY_HERE", "provisionDeviceSecret": "PUT_PROVISION_SECRET_HERE"}'`}
/>
