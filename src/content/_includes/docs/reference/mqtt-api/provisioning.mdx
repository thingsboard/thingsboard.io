...
The MQTT Provisioning API lets a device register itself with ThingsBoard on first boot — before it has any credentials. See <DocLink product={props.product} path='user-guide/device-provisioning'>Device provisioning</DocLink> for the full workflow and how to configure the device profile.
