...
This page documents the legacy `tb-rest-client` package. New projects should use the <DocLink product={props.product} path='reference/python-client'>new Python client</DocLink> — auto-generated from the OpenAPI spec, edition-aware (`tb-ce-client` / `tb-pe-client` / `tb-paas-client`), and actively maintained. The legacy package still works but is no longer recommended for new code.
</Aside>
...
<Code lang="bash" code={`pip3 install tb-rest-client==${PYTHON_CLIENT_VER}`} />
