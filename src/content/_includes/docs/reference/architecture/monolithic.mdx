...
<ImageGallery images={[{ src: '/src/assets/schemas/monolithic-architecture.svg', alt: 'ThingsBoard monolithic architecture: all services running in a single JVM process', caption: 'Transport, Rule Engine, Core, and Actor System all run inside a single JVM. An in-memory queue connects them; PostgreSQL (and optionally Cassandra) is the only external dependency.' }]} />
...
<ImageGallery images={[{ src: '/src/assets/schemas/single-node-vs-cluster.svg', alt: 'ThingsBoard monolithic deployment: single node vs cluster with load balancer and shared infrastructure', caption: 'Single node uses no external queue; cluster mode adds a load balancer, Kafka, Zookeeper, and Redis, with nodes communicating via gRPC.' }]} />
