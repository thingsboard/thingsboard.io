...
<ImageGallery images={[{ src: '/src/assets/schemas/actors-hierarchy.svg', alt: 'ThingsBoard actor hierarchy: App Actor → Tenant Actors → Device, Rule Chain, CF, and Edge Actors', caption: 'The App Actor is the root and supervises one Tenant Actor per tenant. Each Tenant Actor creates Device Actors, Rule Chain Actors (which manage Rule Node Actors), CF Actors, and Edge Actors.' }]} />
