...
<ImageGallery images={[{ src: '/src/assets/schemas/messages-flow.svg', alt: 'ThingsBoard message flow: services communicating through Kafka topics', caption: 'Transport pods push telemetry to the Rule Engine via tb_rule_engine; TB Node communicates with Core via tb_core; JS Executor exchanges script requests and responses via js.eval topics. CF Engine, Edge, and OTA distribution use dedicated topics.' }]} />
