...
Each region runs on managed **Kubernetes**. Application services run within a single Availability Zone (AZ); multiple pod replicas are spread across separate physical racks via the cloud provider's spread placement strategy, so a single-rack failure does not take a service offline. Cassandra replicates time-series data across three AZs and PostgreSQL replicates across two AZs, so data remains durable and accessible if a single AZ becomes unavailable.
...
Long-term data stores replicate across multiple Availability Zones. Other stateful components run within a single AZ with broker/node replicas placed on separate physical racks.
...
| **Apache Kafka** | Topic replication factor 3 within a single AZ; brokers placed on separate physical racks |
| **Redis / Valkey** | Cluster mode within a single AZ; nodes placed on separate physical racks |
...
- Multiple pod replicas run for every service within a single Availability Zone, spread across separate physical racks via the cloud provider's spread placement strategy
