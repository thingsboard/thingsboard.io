...
<Aside type="note">
  The settings listed on this page are ThingsBoard environment variables. To learn where and how to configure these parameters for your deployment type, see <DocLink product={props.product} path="reference/configuration/how-to-change-config" target="_blank">How to change configuration</DocLink>.
</Aside>
...
3. **Entity level** — a specific TTL per entity. Two options let you configure it:
    ...
    Together these enable multi-level retention. Raw telemetry can expire quickly, while derived values produced by calculated fields live on independently, each with the TTL that matches its own analytical purpose. Read more in the <a href="/blog/efficient-aggregation-and-data-retention-in-thingsboard/" target="_blank" rel="noopener noreferrer"><strong>Efficient Aggregation and Data Retention in ThingsBoard</strong></a> blog post.
