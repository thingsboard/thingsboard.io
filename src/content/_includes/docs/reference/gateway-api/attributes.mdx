...
```bash
mosquitto_pub -h "$THINGSBOARD_HOST" -t "v1/gateway/attributes" -u "$ACCESS_TOKEN" -m '{"Device A":{"fw_version":"1.0","battery":87}}'
```
...
```bash
mosquitto_sub -h "$THINGSBOARD_HOST" -t "v1/gateway/attributes/response" -u "$ACCESS_TOKEN" &
mosquitto_pub -h "$THINGSBOARD_HOST" -t "v1/gateway/attributes/request" -u "$ACCESS_TOKEN" -m '{"id":1,"device":"Device A","client":["fw_version"],"shared":["targetFwVersion"]}'
```
