...
Omit `ts` to have the server assign the receive time. In this case, do not use the `values` wrapper — place the telemetry key-value pairs directly inside the array object:
