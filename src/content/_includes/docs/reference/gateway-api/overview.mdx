...
<Aside type="tip">
  ThingsBoard's open-source <DocLink product={props.product} path='iot-gateway/'>IoT Gateway</DocLink> is built on top of this API and is a good reference implementation for building custom gateways.
</Aside>

## Enabling Gateway Mode

Before a device can use the gateway topic namespace, it must have **Is gateway** enabled in ThingsBoard. Toggle this flag when creating the device or in the device edit form — see <DocLink product={props.product} path='user-guide/devices'>Devices</DocLink> for details.
