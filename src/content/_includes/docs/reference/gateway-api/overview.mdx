...
<HostCode product={props.product} lang="bash" code={`mosquitto_pub -d -h "{MQTT_HOST}" -t "v1/gateway/connect" -u "GATEWAY_DEVICE_ACCESS_TOKEN" -m '{"device":"Device A"}' -q 1`} />

Connect with a specific device profile:

<HostCode product={props.product} lang="bash" code={`mosquitto_pub -d -h "{MQTT_HOST}" -t "v1/gateway/connect" -u "GATEWAY_DEVICE_ACCESS_TOKEN" -m '{"device":"Device A","type":"Sensor A"}' -q 1`} />
...
<HostCode product={props.product} lang="bash" code={`mosquitto_pub -d -h "{MQTT_HOST}" -t "v1/gateway/disconnect" -u "GATEWAY_DEVICE_ACCESS_TOKEN" -m '{"device":"Device A"}' -q 1`} />
