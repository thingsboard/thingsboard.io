import DocLink from '@components/DocLink.astro';
import HostCode from '@components/HostCode.astro';
import ShowFor from '@components/ShowFor.astro';
import { Products } from '@models/site.models';

Initiate the claiming process for one or more downstream devices simultaneously.

**Topic:** `v1/gateway/claim`

```json
{
  "Device A": {"secretKey": "secretA", "durationMs": 60000},
  "Device B": {"secretKey": "secretB", "durationMs": 60000}
}
```

| Field | Required | Description |
|-------|----------|-------------|
| `secretKey` | No | Secret key for claiming authorization. Defaults to an empty string. |
| `durationMs` | No | Claiming window in milliseconds. Defaults to the system value of `SECURITY_CLAIM_DURATION`. |

<ShowFor product={props.product} show={[Products.CE, Products.PE]}>

Replace `GATEWAY_DEVICE_ACCESS_TOKEN` in the command below with the gateway device's access token, and `$THINGSBOARD_HOST` with your ThingsBoard instance's host.

</ShowFor>

<ShowFor product={props.product} show={[Products.PAAS, Products.PAAS_EU]}>

Replace `GATEWAY_DEVICE_ACCESS_TOKEN` in the command below with the gateway device's access token.

</ShowFor>

<HostCode product={props.product} lang="bash" code={`mosquitto_pub -h "{MQTT_HOST}" -t "v1/gateway/claim" -u "GATEWAY_DEVICE_ACCESS_TOKEN" -m '{"Device A":{"secretKey":"mySecret","durationMs":60000}}'`} />

For a full explanation of the claiming workflow, see <DocLink product={props.product} path='user-guide/claiming'>Claiming Devices</DocLink>.
