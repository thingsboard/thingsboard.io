...
It renders a T-shaped connector with openings on the left, top, and right — a horizontal run with an upward branch. Each arm carries its own animated fluid flow stripe that can be shown or hidden and controlled in direction and speed from live device data. Use it wherever a main horizontal pipeline splits off an upward branch — for example, routing a horizontal supply line to an elevated tank, heat exchanger, or vent — in HVAC, water treatment, industrial automation, or building management systems.
