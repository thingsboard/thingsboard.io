...
It renders a labeled control panel with two buttons — **On** and **Off** — that operators can click directly on the dashboard to turn a device or subsystem on or off. The panel reads the current state from the device and updates the button appearance accordingly, so the display always reflects reality. Use it to control pumps, fans, heaters, lighting circuits, or any binary-switched equipment in HVAC, water treatment, industrial automation, or building management systems where a clear, labeled toggle control fits the diagram layout.
