...
It renders a vertical scale bar with an arrow pointer that moves to reflect the current sensor value within a defined range. Unlike the HP Simple vertical scale, all four threshold boundaries — high critical, high warning, low warning, and low critical — are **behavior parameters** driven by live device data, so the warning and critical zones on the scale shift in real time as device attributes or telemetry change. An optional target marker shows a setpoint on the same scale. Use it when threshold limits vary per device or installation — for example, temperature operating ranges stored as device attributes — in HVAC, water treatment, industrial automation, or building management systems where a tall portrait-oriented gauge fits the diagram layout.
