...
It represents an industrial manufacturing facility — a factory or production plant connected to an energy or utility network. The widget shows whether the facility is currently in operation (running state) and highlights warning or critical conditions with configurable colors and a blinking animation. Use it on industrial automation, energy management, or smart factory dashboards to visualize plant operational status, surface production or supply alarms, and navigate to facility-level detail views with a single click.
