...
It represents a heat pump with a temperature display, fan rotation animation, a power button for remote start/stop control, and color-coded warning and critical states.
...
Behavior parameters control what the widget displays and what actions it sends to the connected device. See [Read parameter configuration](#read-parameter-configuration) and [Write parameter configuration](#write-parameter-configuration) for the full configuration details.
