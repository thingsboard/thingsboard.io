...
It renders an analog fill-level gauge that shows a liquid level relative to a configurable maximum, with color-coded alarm states.
...
|---|---|---|---|
| **Full value** | `Use attribute 'fullValue'` | number | The value that corresponds to the maximum fill level (100%). Used to calculate the fill percentage. |
| **Value** | `Use time series 'waterLevel'` | number | The current water level value. Compared against **Full value** to determine fill percentage. |
...
| **Broken** | `False` | `True`, `False` | Whether the component is in a broken or fault state. |
| **On display click** | `Do nothing` | — | Action triggered when the user clicks on the display face. |

### On display click

Defines what happens when the user clicks the widget display on the dashboard. Select one action type from the list below.
