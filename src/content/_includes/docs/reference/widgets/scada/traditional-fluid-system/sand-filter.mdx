...
It represents a sand filter with a running state indicator, a set of filtration mode buttons (Filter, Waste, Backwash, Recirculate, Rinse, Closed), and direct mode-switching control from the dashboard.
...
Behavior parameters control what the widget displays and what actions it sends to the connected device. See [Read parameter configuration](#read-parameter-configuration) and [Write parameter configuration](#write-parameter-configuration) for configuration details.
