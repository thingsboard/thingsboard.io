...
The **Pool** widget represents a rectangular pool or open basin with fill level visualization and volume readout.
Use it to monitor liquid volume in fluid system diagrams where a wide, shallow vessel — such as a swimming pool, reservoir, or holding basin — matches the physical equipment.
...
To configure a parameter, click its edit icon. When **Action** is set to **Do nothing**, the **Value** field accepts a static number for both **Tank capacity** and **Current volume**.
