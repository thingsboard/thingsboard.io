...
<Aside type="note">
With **Display pagination** enabled the widget pages through the whole result set, so a long request history is fine.

With it disabled the widget shows one scrollable page of at most **1024 rows**, and requests past that are silently left out.
</Aside>
