...
| Widget | Primary use |
|--------|-------------|
| **Map** | Displays current entity locations in real time. |
| **Image Map** | Uses a custom background image. Best for floor plans, parking lots, and indoor layouts. |
| **Trip Map** | Animated route replay with full timeline controls. Best for step-by-step movement analysis. |
| **Route Map** | Renders the full historical path as a static route line. Best for reviewing complete trips at a glance. |

What sets Route Map apart:

- Draws the route from **timeseries** latitude/longitude data rather than the latest value only.
- Supports the **Polylines** overlay (unique to Route Map and Trip Map) for displaying fixed infrastructure routes like bus lines or pipelines.
...
The time window defines the historical period for which the route is rendered. Use a **relative** window (like "last 24 hours") for a rolling view, or a **fixed** range to inspect a specific trip period.

- **Use dashboard time window:** Inherit the time range from the dashboard. Disable to set a widget-specific range.
- **Display time interval:** Toggle the time interval display on the widget card.
...
<Aside type="note">
Polylines are available only in the Route Map and Trip Map widgets.
</Aside>
...
You can <a href="/resources/route_map_bus_tracking.json" download="route_map_bus_tracking.json">download</a> a preconfigured Route Map widget for this example and reuse it in your own dashboards.
...
2. <a href="/resources/timeseries-map-bus.js" download="timeseries-map-bus.js">Download the emulator script</a>. It connects to `thingsboard.cloud` by default — replace the host with your own ThingsBoard instance URL.
