...
This guide assumes ThingsBoard's UI is served over HTTPS — the standard production setup. Browsers block `http://` resources loaded by an `https://` page (mixed-content protection), so the stream URL must also be HTTPS. The whole point of this guide is to make sure the browser running the dashboard can reach the stream URL directly — either bring your own public HTTPS URL, or deploy MediaMTX + Caddy to put one in front of your camera.
</Aside>
...
The **HLS Video Stream** widget plays any HLS playlist — live or recorded. If you only need VOD, you can skip the MediaMTX and Caddy setup entirely: point the widget's **Fallback stream URL** (or your device's stream URL attribute) at any public `.m3u8` URL. Apple's reference stream is a quick way to try it:
