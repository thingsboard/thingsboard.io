...
The **Alarm count** widget displays the total number of alarms matching specific filter criteria — status, severity, type, or assignee. Use it to put a live alarm KPI on any dashboard: total active alarms, unacknowledged criticals, or alarms assigned to a specific operator. It is commonly placed alongside the <DocLink product={props.product} path="reference/widgets/alarm-widgets/alarms-table" bold={false} target="_blank">Alarms table</DocLink> widget on monitoring dashboards.
