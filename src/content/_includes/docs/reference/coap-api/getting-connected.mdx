...
<Aside type="note">
  Depending on how `libcoap` is packaged, the generic `coap-client` executable may not be installed — only variant-specific binaries such as `coap-client-notls` or `coap-client-openssl`. If `coap-client` is unavailable, use `coap-client-notls` in place of `coap-client` for the plain `coap://` examples in this section.
</Aside>
