...
<HostCode product={props.product} lang="python" meta="maxLines=30 collapsible" code={`import time
import network
from thingsboard_sdk.tb_device_mqtt import TBDeviceMqttClient
...
client = TBDeviceMqttClient("{MQTT_HOST_NAME}", port=1883, access_token="ACCESS_TOKEN")
...
<HostCode product={props.product} lang="python" code={`provision_request = TBDeviceMqttClient.get_provision_request("my_provision_key", "my_provision_secret")
provisioned_credentials = client.provision("{MQTT_HOST_NAME}", 1883, provision_request)
`} />
