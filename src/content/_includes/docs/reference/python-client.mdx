...
The ThingsBoard Python REST Client wraps the <DocLink product={props.product} path='reference/rest-api'>REST API</DocLink> and is auto-generated from the OpenAPI spec — every REST endpoint has a typed method. Edition-specific packages ensure type-safe access to the right endpoints for your tenant.
...
{props.product !== Products.CE && (<Aside type="tip">
...
<Aside type="note">
The methods in this section are only available in the PE and PaaS editions. See the <DocLink product={props.product} path='user-guide/entity-groups'>Entity Groups</DocLink> and <DocLink product={props.product} path='user-guide/role-management'>Roles</DocLink> guides for the underlying concepts.
</Aside>

<ConditionalHeading level={3} id="create-and-populate-a-group" exclude="ce">Create and populate a group</ConditionalHeading>
...
<ConditionalHeading level={3} id="share-with-a-customer" exclude="ce">Share with a customer</ConditionalHeading>
