...
<ConditionalHeading level={2} id="dtls-server-configuration">LwM2M over DTLS — Server configuration</ConditionalHeading>

<ShowFor product={props.product} show={[Products.PAAS, Products.PAAS_EU]}>

ThingsBoard Cloud already has the LwM2M DTLS transport configured, so no server-side setup is required. Connect your device over `coaps://` to <HostInline product={props.product} code="{LWM2M_HOST_LABEL}" /> on port **5686** (server) or **5688** (Bootstrap).
...
<ConditionalHeading level={3} id="supported-dtls-cipher-suites" exclude="paas,paas-eu">Supported DTLS cipher suites</ConditionalHeading>

**With server credentials enabled** (supports PSK, RPK and X.509 client security modes):
...
**Without server credentials** (PSK only):

- `TLS_PSK_WITH_AES_128_CCM_8`
- `TLS_PSK_WITH_AES_128_CBC_SHA256`

The same set applies to the Bootstrap server. All certificate-based suites use ECDSA (not RSA) and AES-128 only. The DTLS role is `SERVER_ONLY` and DTLS 1.2 is used.

Two server-side flags control the offered set:

| Variable | Default | Description |
|----------|---------|-------------|
| `LWM2M_RECOMMENDED_CIPHERS` | `false` | If `true`, restrict to Scandium's "recommended" suites (excludes the CBC variants) |
| `LWM2M_RECOMMENDED_SUPPORTED_GROUPS` | `true` | Restrict supported curves to the recommended set: `secp256r1`, `secp384r1`, `X25519`, `X448` |
