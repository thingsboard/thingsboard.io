...
This experimental server-side API unlocks a variety of potential future uses, but is currently scoped to allow [testing of `.astro` component output](/docs/guides/testing/#vitest-and-container-api) in `vite` environments such as `vitest`.
...
Enables rendering components using [HTML streaming](/docs/guides/on-demand-rendering/#html-streaming).
...
A list of loaded client renderers required by the component. Use this if your `.astro` component renders any [UI framework components](/docs/guides/framework-components/) or MDX using an official Astro integration (e.g. React, Vue, etc.).
...
For [on-demand rendered applications](/docs/guides/on-demand-rendering/), or cases where the container is called at runtime or inside other "shells" (e.g. PHP, Ruby, Java, etc.), renderers must be manually imported.
...
Server renderers are required to build your project, and must be stored in the container for every framework used. Client renderers are additionally needed to any hydrate client-side components using [`client:*` directives](/docs/reference/directives-reference/#client-directives).
...
An option to pass content to be rendered with [`<slots>`](/docs/basics/astro-components/#slots).
...
An option to pass [properties](/docs/basics/astro-components/#component-props) for Astro components.
...
An object to pass information about the path parameter to an Astro component responsible for [generating dynamic routes](/docs/guides/routing/#dynamic-routes).
...
An option to pass information from [`Astro.locals`](/docs/reference/api-reference/#locals) for rendering your component.
...
An option available when using `renderToResponse()` to specify that you are rendering an [endpoint](/docs/guides/endpoints/):
...
Whether or not the Container API renders components as if they were [page partials](/docs/basics/astro-pages/#page-partials). The default `true` setting renders the component in isolation without a full page shell.
