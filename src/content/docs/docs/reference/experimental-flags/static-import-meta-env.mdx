...
Astro allows you to configure a [type-safe schema for your environment variables](/docs/guides/environment-variables/#type-safe-environment-variables), and converts variables imported via `astro:env` into the expected type. This is the recommended way to use environment variables in Astro, as it allows you to easily see and manage whether your variables are public or secret, available on the client or only on the server at build time, and the data type of your values.
...
If you need more control over environment variables in Astro, we recommend you use [`astro:env`](/docs/guides/environment-variables/).
