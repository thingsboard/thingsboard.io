...
Middleware allows you to intercept requests and responses and inject behaviors dynamically every time a page or endpoint is about to be rendered. For features and usage examples, [see our middleware guide](/docs/guides/middleware/).
...
The following helpers can be imported from the regular middleware module when you build an [Astro Integration](/docs/reference/integrations-reference/):
...
**Type:** <code>(context: <a href="#createcontext-1">CreateContext</a>) => <a href="/docs/reference/api-reference/">APIContext</a></code><br />
<Since v="2.8.0" />
</p>

A low-level API to create an [`APIContext`](/docs/reference/api-reference/)to be passed to an Astro middleware [`onRequest()` function](#onrequest).
...
An object containing the optional parameters to be passed to [`Astro.params`](/docs/reference/api-reference/#params).
...
A list of supported locales defined in the [user's `i18n` configuration](/docs/reference/configuration-reference/#i18nlocales).
...
The default locale defined in the [user's `i18n` configuration](/docs/reference/configuration-reference/#i18ndefaultlocale).
...
An object for storing arbitrary information from a middleware, accessible to the user via [`Astro.locals`](/docs/reference/api-reference/#locals).

<ReadMore>Learn more about [storing data in `locals`](/docs/guides/middleware/#storing-data-in-contextlocals) with example usage.</ReadMore>
...
**Type:** <code>(context: <a href="/docs/reference/api-reference/">APIContext</a>, next: <a href="#middlewarenext">MiddlewareNext</a>) => Promise\<Response\> | Response | Promise\<void\> | void</code>
</p>
...
**Type:** [`APIContext`](/docs/reference/api-reference/)
</p>

An [Astro context](/docs/reference/api-reference/) object mirroring many of the `Astro` global properties.
...
Since Astro v4.13.0, `next()` accepts an optional URL path parameter in the form of a string, `URL`, or `Request` to [rewrite](/docs/guides/routing/#rewrites) the current request without retriggering a new rendering phase.
...
Represents the destination for a [rewrite](/docs/guides/routing/#rewrites) when passed to the [`next()`](#next) function.
