...
Astro provides built-in components and helper functions for optimizing and displaying your images. For features and usage examples, [see our image guide](/docs/guides/images/).
...
- **Local images in `src/`** - you must **also import the image** using a relative file path or configure and use an [import alias](/docs/guides/imports/#aliases). Then use the import name as the `src` value:
...
`inferSize` can fetch the dimensions of a [remote image from a domain that has not been authorized](/docs/guides/images/#authorizing-remote-images), however the image itself will remain unprocessed.
...
Defines a [responsive image](#responsive-image-properties) and determines how the image should resize when its container changes size. Can be used to override the default configured value for [`image.layout`](/docs/reference/configuration-reference/#imagelayout).
...
Values match those of CSS `object-fit`. Defaults to `cover`, or the value of [`image.objectFit`](/docs/reference/configuration-reference/#imageobjectfit) if set. Can be used to override the default `object-fit` styles. 
...
Values match those of CSS `object-position`. Defaults to `center`, or the value of [`image.objectPosition`](/docs/reference/configuration-reference/#imageobjectposition) if set. Can be used to override the default `object-position` styles. 
...
The `getImage()` function is intended for generating images destined to be used somewhere else than directly in HTML, for example in an [API Route](/docs/guides/endpoints/#server-endpoints-api-routes). It also allows you to create your own custom `<Image />` component.
...
Retrieves the resolved [image service](/docs/reference/configuration-reference/#imageservice).

### `imageConfig`

<p>

**Type:** [`AstroConfig["image"]`](/docs/reference/configuration-reference/#image-options)<br />
<Since v="3.0.9" />
</p>

The [configuration options for images](/docs/reference/configuration-reference/#image-options) set by the user and merged with all defaults.
...
<ReadMore>Learn more about [imported images in `src/`](/docs/guides/images/#images-in-src) with an example usage.</ReadMore>
...
The built-in local image service which can be extended to [create a custom image service](/docs/reference/image-service-reference/).
...
**Type:** <code>(options: <a href="#unresolvedimagetransform">UnresolvedImageTransform</a>, imageConfig: <a href="/docs/reference/configuration-reference/#image-options">AstroConfig['image']</a>) => Promise\<<a href="#getimageresult">GetImageResult</a>\></code>
</p>

A function similar to [`getImage()`](#getimage) from `astro:assets` with two required arguments: an `options` object with [the same properties as the Image component](#image-) and a second object for the [image configuration](/docs/reference/configuration-reference/#image-options).
...
The following helpers are imported from the `utils` directory in the regular assets module and can be used to [build an image service](/docs/reference/image-service-reference/):
...
Describes the result of the transformation after the call to [`getImage()`](/docs/reference/modules/astro-assets/#getimage).
...
<ReadMore>See also the [`inferSize` attribute](/docs/reference/modules/astro-assets/#infersize) available on `<Image />`.</ReadMore>
...
Defines the hooks that an external image transformation service must provide. This requires a [`getUrl()` hook](/docs/reference/image-service-reference/#geturl) and supports [three additional hooks](/docs/reference/image-service-reference/#hooks).

<ReadMore>Learn how to build [external services](/docs/reference/image-service-reference/#external-services) in the Image Service API reference with example usage.</ReadMore>
...
Defines the hooks that a local image transformation service must provide. This requires [`getUrl()`](/docs/reference/image-service-reference/#geturl), [`parseUrl()`](/docs/reference/image-service-reference/#parseurl), and [`transform()`](/docs/reference/image-service-reference/#transform)  hooks, and supports [additional hooks](/docs/reference/image-service-reference/#hooks).

<ReadMore>Learn how to build [local services](/docs/reference/image-service-reference/#local-services) in the Image Service API reference with example usage.</ReadMore>
