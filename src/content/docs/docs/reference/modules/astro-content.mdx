...
Content collections offer APIs to configure and query your Markdown or MDX documents in `src/content/`. For features and usage examples, [see our content collections guide](/docs/guides/content-collections/).
...
**Type:** <code>() => Promise&lt;Array&lt;\{ id: string, [key: string]: any }&gt; | Record&lt;string, Record&lt;string, any&gt;&gt;&gt; | <a href="/docs/reference/content-loader-reference/#object-loader-api">Loader</a></code>
<Since v="5.0.0" />
</p>
...
[See the `Content Collection` guide](/docs/guides/content-collections/#defining-the-collection-loader) for example usage.
...
`schema` is an optional Zod object to configure the type and shape of document frontmatter for a collection. Each value must use [a Zod validator](/docs/reference/modules/astro-zod/#common-data-type-validators).

[See the `Content Collection` guide](/docs/guides/content-collections/#defining-the-collection-schema) for example usage.
...
[See the `Content Collection` guide](/docs/guides/content-collections/#defining-collection-references) for example usage.
...
[See the `Content Collection` guide](/docs/guides/content-collections/#querying-collections) for example usage.
...
See the `Content Collections` guide for examples of [querying collection entries](/docs/guides/content-collections/#querying-collections).
...
`getEntries()` is a function that retrieves multiple collection entries from the same collection. This is useful for [returning an array of referenced entries](/docs/guides/content-collections/#defining-collection-references) to access their associated `data` and `body` properties.
...
- `headings` - A generated list of headings, [mirroring Astro's `getHeadings()` utility](/docs/guides/markdown-content/#available-properties) on Markdown and MDX imports.
- `remarkPluginFrontmatter ` - The modified frontmatter object after any [remark or rehype plugins have been applied](/docs/guides/markdown-content/#modifying-frontmatter-programmatically). Set to type `any`.
...
[See the `Content Collection` guide](/docs/guides/content-collections/#rendering-body-content) for example usage.
...
- `image` - The `image()` schema helper that allows you [to use local images in Content Collections](/docs/guides/images/#images-in-content-collections)
