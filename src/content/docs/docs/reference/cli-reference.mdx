...
The following scripts for the most common `astro` commands (`astro dev`, `astro build`, and `astro preview`) are added for you automatically when you create a project using [the `create astro` wizard](/docs/install-and-setup/).

When you follow the instructions to [install Astro manually](/docs/install-and-setup/#manual-setup), you are instructed to add these scripts yourself. You can also add more scripts to this list manually for any commands you use frequently.
...
Builds your site for deployment. By default, this will generate static files and place them in a `dist/` directory. If any routes are [rendered on demand](/docs/guides/on-demand-rendering/), this will generate the necessary server files to serve your site.
...
This command allows you to preview your site locally [after building](#astro-build) to catch any errors in your build output before deploying it. It is not designed to be run in production. For help with production hosting, check out our guide on [Deploying an Astro Website](/docs/guides/deploy/).
...
<ReadMore>Read more about [type checking in Astro](/docs/guides/typescript/#type-checking).</ReadMore>
...
Generates TypeScript types for all Astro modules. This sets up a [`.astro/types.d.ts` file](/docs/guides/typescript/#setup) for type inferencing, and defines modules for features that rely on generated types:
- The `astro:content` module for the [Content Collections API](/docs/guides/content-collections/).
- The `astro:db` module for [Astro DB](/docs/guides/astro-db/).
- The `astro:env` module for [Astro Env](/docs/guides/environment-variables/).
- The `astro:actions` module for [Astro Actions](/docs/guides/actions/)

## `astro add`

Adds an integration to your configuration. Read more in [the integrations guide](/docs/guides/integrations-guide/#automatic-integration-setup).
...
Set this key as the `ASTRO_KEY` environment variable (e.g. in a `.env` file) and include it in your CI/CD or host’s build settings when you need [a constant encryption key for your server islands](/docs/guides/server-islands/#reusing-the-encryption-key) for situations like rolling deployments, multi-region hosting or a CDN that caches pages containing server islands.
...
Clear the [content layer cache](/docs/guides/content-collections/#defining-the-collection-loader), forcing a full rebuild.

### `--mode <string>`

<p><Since v="5.0.0" /></p>

Configures the [`mode`](/docs/reference/programmatic-reference/#mode) inline config for your project.

### `--outDir <path>`

<p><Since v="3.3.0" /></p>

Configures the [`outDir`](/docs/reference/configuration-reference/#outdir) for your project. Passing this flag will override the `outDir` value in your `astro.config.mjs` file, if one exists.

### `--site <url>`

Configures the [`site`](/docs/reference/configuration-reference/#site) for your project. Passing this flag will override the `site` value in your `astro.config.mjs` file, if one exists.

### `--base <pathname>`

<p><Since v="1.4.1" /></p>

Configures the [`base`](/docs/reference/configuration-reference/#base) for your project. Passing this flag will override the `base` value in your `astro.config.mjs` file, if one exists.
