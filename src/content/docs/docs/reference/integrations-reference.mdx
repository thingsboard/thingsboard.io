...
This reference page is for anyone writing their own integration. To learn how to use an integration in your project, check out our [Using Integrations](/docs/guides/integrations-guide/) guide instead.

## Examples

The official Astro integrations can act as reference for you as you go to build your own integrations.

- **Renderers:** [`svelte`](/docs/guides/integrations-guide/svelte/), [`react`](/docs/guides/integrations-guide/react/), [`preact`](/docs/guides/integrations-guide/preact/), [`vue`](/docs/guides/integrations-guide/vue/), [`solid`](/docs/guides/integrations-guide/solid-js/)
- **Libraries:** [`partytown`](/docs/guides/integrations-guide/partytown/)
- **Features:** [`sitemap`](/docs/guides/integrations-guide/sitemap/)
...
**When:** On initialization, before either the [Vite](https://vite.dev/config/) or [Astro config](/docs/reference/configuration-reference/) have resolved.

**Why:** To extend the project config. This includes updating the [Astro config](/docs/reference/configuration-reference/), applying [Vite plugins](https://vite.dev/guide/api-plugin.html), adding component renderers, and injecting scripts onto the page.
...
A read-only copy of the user-supplied [Astro config](/docs/reference/configuration-reference/). This is resolved _before_ any other integrations have run. If you need a copy of the config after all integrations have completed their config updates, [see the `astro:config:done` hook](#astroconfigdone).
...
A callback function to update the user-supplied [Astro config](/docs/reference/configuration-reference/). Any config you provide **will be merged with the user config + other integration config updates,** so you are free to omit keys!
...
Adds a [custom client directive](/docs/reference/directives-reference/#custom-client-directives) to be used in `.astro` files.
...
Adds a [custom dev toolbar app](/docs/reference/dev-toolbar-app-reference/).
...
Adds [middleware](/docs/guides/middleware/) to run on each request. Takes the `entrypoint` module that contains the middleware, and an `order` to specify whether it should run before (`pre`) other middleware or after (`post`).
...
Middleware is defined in a package with an [`onRequest()` function](/docs/reference/modules/astro-middleware/#onrequest), as with user-defined middleware.
...
A callback function to inject routes into an Astro project. Injected routes can be [`.astro` pages](/docs/basics/astro-pages/) or [`.js` and `.ts` route handlers](/docs/guides/endpoints/#static-file-endpoints).
...
**Why:** To set options for a route at build or request time, such as enabling [on-demand server rendering](/docs/guides/on-demand-rendering/#enabling-on-demand-rendering).
...
The `prerender` property is used to configure [on-demand server rendering](/docs/guides/on-demand-rendering/#enabling-on-demand-rendering) for a route. If the route file contains an explicit `export const prerender` value, the value will be used as the default instead of `undefined`.
...
If the final value after running all the hooks is `undefined`, the route will fall back to a prerender default based on the [`output` option](/docs/reference/configuration-reference/#output): prerendered for `static` mode, and on-demand rendered for `server` mode.
...
A read-only copy of the user-supplied [Astro config](/docs/reference/configuration-reference/). This is resolved _after_ other integrations have run.
...
Makes the integration an adapter. Read more in the [adapter API](/docs/reference/adapter-reference/).
...
A mutable instance of the Vite server used in "dev" mode. For instance, this is [used by our Partytown integration](/docs/guides/integrations-guide/partytown/) to inject the Partytown server as middleware:
...
An object providing callback functions to interact with the [dev toolbar](/docs/reference/dev-toolbar-app-reference/):
...
The loader can then access the `refreshContextData` property to get the webhook body. See the [`refreshContextData`](/docs/reference/content-loader-reference/#refreshcontextdata) property for more information.
...
**Why:** To set up any global objects or clients needed during a production build. This can also extend the build configuration options in the [adapter API](/docs/reference/adapter-reference/).
...
Exposes the [middleware](/docs/guides/middleware/) file path.
...
:::caution
This property is deprecated since v5.0. Check the [migration guide](/docs/guides/upgrade-to/v5/#deprecated-routes-on-astrobuilddone-hook-integration-api).
:::
...
An instance of the Astro logger, useful to write logs. This logger uses the same [log level](/docs/reference/cli-reference/#--verbose)
configured via CLI.
...
**Type:** <code>\{ onRequest?: <a href="/docs/reference/modules/astro-middleware/#middlewarehandler">MiddlewareHandler</a>; \}</code>
</p>

An object containing an [`onRequest()`](/docs/reference/modules/astro-middleware/#onrequest) property defined with the project's middleware function when it exists.
...
Determines whether the route use [on demand rendering](/docs/guides/on-demand-rendering/). The value will be `true` for projects configured with:
...
Allows you to access the route `params`. For example, when a project uses the following [dynamic routes](/docs/guides/routing/#dynamic-routes) `/pages/[lang]/[...slug].astro`, the value will be `['lang', '...slug']`.
...
For regular routes, the value will be the URL pathname where this route will be served. When the project uses [dynamic routes](/docs/guides/routing/#dynamic-routes) (ie. `[dynamic]` or `[...spread]`), the pathname will be undefined.
...
Determines whether a route uses [on demand rendering](/docs/guides/on-demand-rendering/) or is statically prerendered at build time.

See also [`prerendered`](/docs/reference/routing-reference/#prerender) in the routing reference.
...
Defines a list of `RouteData` to fallback to when [`i18n.fallback`](/docs/reference/configuration-reference/#i18nfallback) has a list of locales.
...
Defines the name of the [server adapter](/docs/guides/on-demand-rendering/#server-adapters) used for on-demand rendering.
...
Specifies the [configured `site`](/docs/reference/configuration-reference/#site).
...
Specifies the [configured `base` path](/docs/reference/configuration-reference/#base) to deploy to.
...
**Type:** [`AstroConfig['trailingSlash']`](/docs/reference/configuration-reference/#trailingslash)<br />
<Since v="3.5.4" />
</p>

Specifies the [configured behavior for trailing slashes](/docs/reference/configuration-reference/#trailingslash) in development mode and for on-demand rendered pages.

#### `SSRManifest.buildFormat`

<p>

**Type:** [`NonNullable<AstroConfig['build']>['format']`](/docs/reference/configuration-reference/#buildformat)<br />
<Since v="4.2.2" />
</p>

Specifies the [configured output file format](/docs/reference/configuration-reference/#buildformat).
...
Determines whether [HTML minification is enabled in the project configuration](/docs/reference/configuration-reference/#compresshtml).
...
Specifies the [configured prefix for Astro-generated asset links](/docs/reference/configuration-reference/#buildassetsprefix).
...
Defines a mapping of client directive names (e.g. `load`, `visible`) to their implementation code. This includes both [built-in client directives](/docs/reference/directives-reference/#client-directives) and [custom client directives](/docs/reference/directives-reference/#custom-client-directives).
...
**Type:** [`MiddlewareHandler`](/docs/reference/modules/astro-middleware/#middlewarehandler)<br />
<Since v="3.0.3" />
</p>

An [Astro middleware function](/docs/reference/modules/astro-middleware/#onrequest) when defined in the user project.
...
Specifies the resolved [`i18n` configuration](/docs/reference/configuration-reference/#i18n) when enabled in the project.
...
Defines the [i18n routing strategy](/docs/reference/configuration-reference/#i18nrouting) configured. This determines how locales are handled in URLs and whether redirects occur.
...
Specifies a list of [supported locales configured in the project](/docs/reference/configuration-reference/#i18nlocales).
...
Determines the [default locale configured in the project](/docs/reference/configuration-reference/#i18ndefaultlocale).
...
Specifies a mapping of locales to their fallback locales as [configured in `i18n.fallback`](/docs/reference/configuration-reference/#i18nfallback).
...
Determines the [configured fallback strategy for the project](/docs/reference/configuration-reference/#i18nroutingfallbacktype).
...
A mapping of [configured domains](/docs/reference/configuration-reference/#i18ndomains) to their associated locales.
...
**Type:** <code>() => Promise\<\{ server: Record\<string, <a href="/docs/reference/modules/astro-actions/#actionclient">ActionClient</a>\>; \}\> | \{ server: Record\<string, <a href="/docs/reference/modules/astro-actions/#actionclient">ActionClient</a>\>; \}</code><br />
<Since v="5.4.2" />
</p>
...
Determines whether [origin checking is enabled in the security configuration](/docs/reference/configuration-reference/#securitycheckorigin).

#### `SSRManifest.allowedDomains`

<p>

**Type:** <code>Partial\<<a href="/docs/reference/modules/astro-assets/#remotepattern">RemotePattern</a>\>[]</code>
</p>

Specifies the [configured list of permitted host patterns](/docs/reference/configuration-reference/#securityalloweddomains) for incoming requests when using on-demand rendering.

#### `SSRManifest.sessionConfig`

<p>

**Type:** <code><a href="/docs/reference/configuration-reference/#session-options">SessionConfig\<TDriver\></a> & \{ driverModule?: () => Promise\<\{ default: () => unstorage.Driver \}\>; }</code><br />
<Since v="5.1.0" />
</p>

An object containing the [resolved session configuration](/docs/reference/configuration-reference/#session-options) and an additional property defining the driver in use.
...
Specifies the [configured directory for caching build artifacts](/docs/reference/configuration-reference/#cachedir).
...
Specifies the [configured directory that Astro will read the site from](/docs/reference/configuration-reference/#srcdir).
...
Specifies the [configured directory in which to write the final build](/docs/reference/configuration-reference/#outdir).
...
Specifies the [configured directory for the static assets](/docs/reference/configuration-reference/#publicdir).
...
Describes the [Content Security Policy configuration](/docs/reference/experimental-flags/csp/).
...
Specifies whether CSP directives should be injected as a `meta` element, as a response `header`, or by the [`adapter` when it supports setting response headers](/docs/reference/adapter-reference/#experimentalstaticheaders).
...
Specifies the [configured hash function](/docs/reference/experimental-flags/csp/#algorithm).
...
Specifies a list of generated hashes for project scripts and [user-supplied hashes](/docs/reference/experimental-flags/csp/#hashes) for external scripts.
...
Specifies a list of valid sources combining the [configured script resources](/docs/reference/experimental-flags/csp/#resources) and the [injected script resources](/docs/reference/experimental-flags/csp/#cspinsertscriptresource).
...
Determines whether support for [dynamic script injection is enabled in the configuration](/docs/reference/experimental-flags/csp/#strictdynamic).
...
Specifies a list of generated hashes for project styles and [user-supplied hashes](/docs/reference/experimental-flags/csp/#hashes) for external styles.
...
Specifies a list of valid sources combining the [configured style resources](/docs/reference/experimental-flags/csp/#resources) and the [injected style resources](/docs/reference/experimental-flags/csp/#cspinsertstyleresource).
...
Specifies the [configured list of valid sources](/docs/reference/experimental-flags/csp/#directives) for specific content types.
...
[The `astro add` command](/docs/reference/cli-reference/#astro-add) allows users to easily add integrations and adapters to their project. If you want _your_ integration to be installable with this tool, **add `astro-integration` to the `keywords` field in your `package.json`**:
