...
Every [supported page file](/docs/basics/astro-pages/#supported-page-files) located within the special `src/pages/` directory creates a route. When the file name contains a [parameter](#params), a route can create multiple pages dynamically, otherwise it creates a single page.

By default, all Astro page routes and endpoints are generated and prerendered at build time. [On-demand server rendering](/docs/guides/on-demand-rendering/) can be set for individual routes, or as the default.
...
You can override the default value to enable [on demand rendering](/docs/guides/on-demand-rendering/) for an individual route by exporting `prerender` with the value `false` from that file:
...
You can override the default value for all routes by configuring [`output: 'server'`](/docs/reference/configuration-reference/#output). In this output mode, all pages and endpoints will be generated on the server upon request by default instead of being prerendered.
...
You can override the default value to designate the content as a [page partial](/docs/basics/astro-pages/#page-partials) for an individual route by exporting a value for `partial` from that file:
...
`getStaticPaths()` can also be used in static file endpoints for [dynamic routing](/docs/guides/endpoints/#params-and-dynamic-routing).

:::tip
When using TypeScript, use the [`GetStaticPaths`](/docs/guides/typescript/#infer-getstaticpaths-types) type utility to ensure type-safe access of your `params` and `props`.
:::
...
A property available in [`getStaticPaths()`](#getstaticpaths) options to access the current [`routePattern`](/docs/reference/api-reference/#routepattern) as a string.

This provides data from the [Astro render context](/docs/reference/api-reference/) that would not otherwise be available within the scope of `getStaticPaths()` and can be useful to calculate the `params` and `props` for each page route.
...
Get the URL of the current page (useful for canonical URLs). If a value is set for [`base`](/docs/reference/configuration-reference/#base), the URL starts with that value.
...
Get the URL of the previous page (will be `undefined` if on page 1). If a value is set for [`base`](/docs/reference/configuration-reference/#base), prepend the base path to the URL.
...
Get the URL of the next page (will be `undefined` if no more pages). If a value is set for [`base`](/docs/reference/configuration-reference/#base), prepend the base path to the URL.
...
Get the URL of the first page (will be `undefined` if on page 1). If a value is set for [`base`](/docs/reference/configuration-reference/#base), prepend the base path to the URL.
...
Get the URL of the last page (will be `undefined` if no more pages). If a value is set for [`base`](/docs/reference/configuration-reference/#base), prepend the base path to the URL.
