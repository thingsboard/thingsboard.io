...
These directives control how [UI Framework components](/docs/guides/framework-components/) are hydrated on the page.
...
A client directive can only be used on a UI framework component that is directly imported into a `.astro` component.  Hydration directives are not supported when using [dynamic tags](/docs/reference/astro-syntax/#dynamic-tags) and [custom components passed via the `components` prop](/docs/guides/integrations-guide/mdx/#passing-components-to-mdx-content).
...
Visit the [`addClientDirective` API](/docs/reference/integrations-reference/#addclientdirective-option) page to learn more about creating a custom client directive.
...
<ReadMore>See more about using [server island components](/docs/guides/server-islands/).</ReadMore>
...
<ReadMore>See the [Styling & CSS](/docs/guides/styling/#global-styles) page for more details about how global styles work.</ReadMore>
...
The one exception is using the [`define:vars` directive](/docs/reference/directives-reference/#definevars) on the `<style>` tag, which does not automatically imply `is:inline`.
:::
...
<ReadMore>See how [client-side scripts](/docs/guides/client-side-scripts/) work in Astro components.</ReadMore>
...
For scripts, try [passing variables to scripts manually](/docs/guides/client-side-scripts/#pass-frontmatter-variables-to-scripts) instead.
:::
