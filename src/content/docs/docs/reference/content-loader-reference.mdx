...
Astro's  Content Loader API allows you to load your data from any source, local or remote, and interact with Astro's content layer to manage your [content collections](/docs/guides/content-collections/).

## What is a loader?

Astro loaders allow you to load data into [content collections](/docs/guides/content-collections/), which can then be used in pages and components. The [built-in `glob()` and `file()` loaders](/docs/guides/content-collections/#built-in-loaders) are used to load content from the file system, and you can create your own loaders to load content from other sources.

Each collection needs [a loader defined in its schema](/docs/guides/content-collections/#defining-the-collection-loader). You can define a loader inline in your project's `src/content.config.ts` file, share one loader between multiple collections, or even [publish your loader to NPM as a package](/docs/reference/publish-to-npm/) to share with others and be included in our integrations library.
...
A callback function to create a collection from a file’s contents. Use it when you need to process file not supported by default (e.g. `.csv`) or when using [nested `.json` documents](/docs/guides/content-collections/#nested-json-documents).
...
A unique name for the loader, used in logs and [for conditional loading](/docs/reference/integrations-reference/#refreshcontent-option).
...
An optional [Zod schema](/docs/guides/content-collections/#defining-datatypes-with-zod) that defines the shape of the entries. It is used to both validate the data and also to generate TypeScript types for the collection.
...
**Type**: [`AstroIntegrationLogger`](/docs/reference/integrations-reference/#astrointegrationlogger)
</p>

A logger that can be used to log messages to the console. Use this instead of `console.log` for more helpful logs that include the loader name in the log message. See [`AstroIntegrationLogger`](/docs/reference/integrations-reference/#astrointegrationlogger) for more information.
...
The full, resolved Astro configuration object with all defaults applied. See [the configuration reference](/docs/reference/configuration-reference/) for more information.
...
This allows you to render Markdown content directly within your loaders using the same Markdown processing as Astro's built-in `glob` loader and provides access to the `render()` function and `<Content />` component for [rendering body content](/docs/guides/content-collections/#rendering-body-content).

Assign this object to the [rendered](#rendered) field of the [DataEntry](#dataentry) object to allow users to [render the content in a page](/docs/guides/content-collections/#rendering-body-content).
...
If the loader has been triggered by an integration, this may optionally contain extra data set by that integration. It is only set when the loader is triggered by an integration. See the [`astro:server:setup`](/docs/reference/integrations-reference/#refreshcontent-option) hook reference for more information.
...
If not set, then any fields in the schema that use [the `image()` helper](/docs/guides/images/#images-in-content-collections) will be treated as [public paths](/docs/guides/images/#where-to-store-images) and not transformed.
...
If this field is provided, then [the `render()` function and `<Content />` component](/docs/guides/content-collections/#rendering-body-content) are available to render the entry in a page.
