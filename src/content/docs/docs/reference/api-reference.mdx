...
The `Astro` global object is available to all `.astro` files. Use the `context` object in [endpoint functions](/docs/guides/endpoints/) to serve static or live server endpoints and in [middleware](/docs/guides/middleware/) to inject behavior when a page or endpoint is about to be rendered.
...
`props` is an object containing any values that have been passed as [component attributes](/docs/basics/astro-components/#component-props).
...
<ReadMore>Learn more about how [Markdown and MDX layouts](/docs/guides/markdown-content/#frontmatter-layout-property) handle props.</ReadMore>
...
See also: [Data Passing with `props`](/docs/reference/routing-reference/#data-passing-with-props)

### `params`

`params` is an object containing the values of dynamic route segments matched for a request. Its keys must match the [parameters](/docs/guides/routing/#dynamic-routes) in the page or endpoint file path.

In static builds, this will be the `params` returned by `getStaticPaths()` used for prerendering [dynamic routes](/docs/guides/routing/#dynamic-routes):
...
See also: [`params`](/docs/reference/routing-reference/#params)
...
`url` will be a `localhost` URL in dev mode. When building a site, prerendered routes will receive a URL based on the [`site`](/docs/reference/configuration-reference/#site) and [`base`](/docs/reference/configuration-reference/#base) options. If `site` is not configured, prerendered pages will receive a `localhost` URL during builds as well.
...
`site` returns a `URL` made from `site` in your Astro config. It returns `undefined` if you have not set a value for [`site`](/docs/reference/configuration-reference/#site) in your Astro config.
...
It is computed by checking the configured locales in your [`i18n.locales`](/docs/reference/configuration-reference/#i18nlocales) array and the locales supported by the user's browser via the header `Accept-Language`. This value is `undefined` if no such match exists.
...
If the browser does not specify any preferred languages, then this value will be [`i18n.locales`](/docs/reference/configuration-reference/#i18nlocales): all of your supported locales will be considered equally preferred by a visitor with no preferences. 
...
The locale computed from the current URL, using the syntax specified in your `locales` configuration. If the URL does not contain a `/[locale]/` prefix, then the value will default to [`i18n.defaultLocale`](/docs/reference/configuration-reference/#i18ndefaultlocale).
...
`getActionResult()` is a function that returns the result of an [Action](/docs/guides/actions/) submission. This accepts an action function as an argument (e.g. `actions.logout`) and returns a `data` or `error` object when a submission is received. Otherwise, it will return `undefined`.
...
`cookies` contains utilities for reading and manipulating cookies for [routes rendered on demand](/docs/guides/on-demand-rendering/).
...
`session` is an object that allows data to be stored between requests for [routes rendered on demand](/docs/guides/on-demand-rendering/). It is associated with a cookie that contains the session ID only: the data itself is not stored in the cookie.
...
See [the session guide](/docs/guides/sessions/) for more information on how to use sessions in your Astro project.
...
See the [imports guide](/docs/guides/imports/#importmetaglob) for more information and usage.
:::
...
`.glob()` can't take variables or strings that interpolate them, as they aren't statically analyzable. (See [the imports guide](/docs/guides/imports/#supported-values) for a workaround.) This is because `Astro.glob()` is a wrapper of Vite's [`import.meta.glob()`](https://vite.dev/guide/features.html#glob-import).
