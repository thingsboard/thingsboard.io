...
2. Set up `src/pages/index.astro` to redirect to your default language.
    ...
    <StaticSsrTabs>
      ...
      <Fragment slot="ssr">
        If you are using an SSR adapter, you can use [`Astro.redirect`](/docs/guides/routing/#dynamic-redirects) to redirect to the default language on the server.
...
2. Create a `src/content.config.ts` file and export a collection for each type of content.
    ...
    <ReadMore>Read more about [Content Collections](/docs/guides/content-collections/).</ReadMore>

3. Use [dynamic routes](/docs/guides/routing/#dynamic-routes) to fetch and render content based on a `lang` and a `slug` parameter.
    ...
    <StaticSsrTabs>
      ...
      <Fragment slot="ssr">
        In [SSR mode](/docs/guides/on-demand-rendering/), fetch the requested entry directly:
    ...
    <ReadMore>Read more about [dynamic routing](/docs/guides/routing/#dynamic-routes).</ReadMore>
...
4. Each page must have a `lang` attribute on the `<html>` element that matches the language on the page. In this example, a [reusable layout](/docs/basics/layouts/) extracts the language from the current route:
