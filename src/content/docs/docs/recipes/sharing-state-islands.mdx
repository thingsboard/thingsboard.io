...
When building an Astro website with [islands architecture / partial hydration](/docs/concepts/islands/), you may have run into this problem: **I want to share state between my components.**

UI frameworks like React or Vue may encourage ["context" providers](https://react.dev/learn/passing-data-deeply-with-context) for other components to consume. But when [partially hydrating components](/docs/guides/framework-components/#hydrating-interactive-components) within Astro or Markdown, you can't use these context wrappers.
...
Nano Stores can be used in `<script>` tags to [share state between `.astro` components](/docs/recipes/sharing-state/). However, Using Nano Stores in the frontmatter of server-side components is not recommended because of the following restrictions:
- Writing to a store from a `.astro` file or [non-hydrated component](/docs/guides/framework-components/#hydrating-interactive-components) will _not_ affect the value received by [client-side components](/docs/reference/directives-reference/#client-directives).
