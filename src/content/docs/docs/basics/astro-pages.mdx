...
- `.mdx` (with the [MDX Integration installed](/docs/guides/integrations-guide/mdx/#installation))
- [`.html`](#html-pages)
- `.js`/`.ts` (as [endpoints](/docs/guides/endpoints/))

## File-based routing

Astro leverages a routing strategy called **file-based routing**. Each file in your `src/pages/` directory becomes an endpoint on your site based on its file path.

A single file can also generate multiple pages using [dynamic routing](/docs/guides/routing/#dynamic-routes). This allows you to create pages even if your content lives outside of the special `/pages/` directory, such as in a [content collection](/docs/guides/content-collections/) or a [CMS](/docs/guides/cms/).

<ReadMore>Read more about [Routing in Astro](/docs/guides/routing/).</ReadMore>
...
Astro pages use the `.astro` file extension and support the same features as [Astro components](/docs/basics/astro-components/).
...
To avoid repeating the same HTML elements on every page, you can move common `<head>` and `<body>` elements into your own [layout components](/docs/basics/layouts/). You can use as many or as few layout components as you'd like.
...
<ReadMore>Read more about [layout components](/docs/basics/layouts/) in Astro.</ReadMore>

## Markdown/MDX Pages

Astro also treats any Markdown (`.md`) files inside of `src/pages/` as pages in your final website. If you have the [MDX Integration installed](/docs/guides/integrations-guide/mdx/#installation), it also treats MDX (`.mdx`) files the same way.

:::tip
Consider creating [content collections](/docs/guides/content-collections/) instead of pages for directories of related Markdown files that share a similar structure, such as blog posts or product items.
:::

Markdown files can use the special `layout` frontmatter property to specify a [layout component](/docs/basics/layouts/) that will wrap their Markdown content in a full `<html>...</html>` page document.
...
<ReadMore>Read more about [Markdown](/docs/guides/markdown-content/) in Astro.</ReadMore>

## HTML Pages

Files with the `.html` file extension can be placed in the `src/pages/` directory and used directly as pages on your site. Note that some key Astro features are not supported in [HTML Components](/docs/basics/astro-components/#html-components).

## Custom 404 Error Page

For a custom 404 error page, you can create a `404.astro` or `404.md` file in `src/pages`.

This will build to a `404.html` page. Most [deploy services](/docs/guides/deploy/) will find and use it.

## Custom 500 Error Page

For a custom 500 error page to show for pages that are [rendered on demand](/docs/guides/on-demand-rendering/), create the file `src/pages/500.astro`. This custom page is not available for prerendered pages.
...
Partials, when combined with a rendering library, provide an alternative to [Astro islands](/docs/concepts/islands/) and [`<script>` tags](/docs/guides/client-side-scripts/) for building dynamic content in Astro.

Page files that can export a value for [`partial`](/docs/reference/routing-reference/#partial) (e.g. `.astro` and `.mdx`, but not `.md`) can be marked as partials.
