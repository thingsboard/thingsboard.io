...
<ReadMore>Learn more in the [Astro syntax reference](/docs/reference/astro-syntax/).</ReadMore>
:::
...
When your Astro component does need client-side interactivity, you can add [standard HTML `<script>` tags](/docs/guides/client-side-scripts/) or [UI Framework components](/docs/guides/framework-components/#hydrating-interactive-components) as "client islands".

For components that need to render personalized or dynamic content, you can defer their server rendering by adding a [server directive](/docs/reference/directives-reference/#server-directives). These "server islands" will render their content when it is available, without delaying the entire page load.
...
<ReadMore>Read more about Astro's [built-in TypeScript support](/docs/guides/typescript/).</ReadMore>
:::
...
However, [Astro's component template syntax](/docs/reference/astro-syntax/) also supports **JavaScript expressions**, Astro [`<style>`](/docs/guides/styling/#styling-in-astro) and [`<script>`](/docs/guides/client-side-scripts/) tags, **imported components**, and [**special Astro directives**](/docs/reference/directives-reference/). Data and values defined in the component script can be used in the component template to produce dynamically-created HTML.
...
This pattern is the basis of an [Astro layout component](/docs/basics/layouts/): an entire page of HTML content can be “wrapped” with `<SomeLayoutComponent></SomeLayoutComponent>` tags and sent to the component to render inside of common page elements defined there.

<ReadMore>See the [`Astro.slots` utility functions](/docs/reference/astro-syntax/#astroslots) for more ways to access and render slot content.</ReadMore>
...
To pass multiple HTML elements into a component's `<slot/>` placeholder without a wrapping `<div>`, use the `slot=""` attribute on [Astro's `<Fragment/>` component](/docs/reference/astro-syntax/#fragments):
...
:::tip
Named slots can also be passed to [UI framework components](/docs/guides/framework-components/)!
:::
...
- Any `<script>` tags are left unbundled, treated as if they had an [`is:inline` directive](/docs/reference/directives-reference/#isinline). 
- They can only [reference assets that are in the `public/` folder](/docs/basics/project-structure/#public).

:::note
A [`<slot />` element](/docs/basics/astro-components/#slots) inside an HTML component will work as it would in an Astro component. In order to use the [HTML Web Component Slot](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/slot) element instead, add `is:inline` to your `<slot>` element.
:::

## Next Steps

<ReadMore>Read more about using [UI framework components](/docs/guides/framework-components/) in your Astro project.</ReadMore>
