...
This adapter allows Astro to deploy your [on-demand rendered routes and features](/docs/guides/on-demand-rendering/) to Node targets, including [server islands](/docs/guides/server-islands/), [actions](/docs/guides/actions/), and [sessions](/docs/guides/sessions/).
...
Disables Astro's default [HTML streaming](/docs/guides/on-demand-rendering/#html-streaming) for pages rendered on demand.
...
For example, when [experimental Content Security Policy](/docs/reference/experimental-flags/csp/) is enabled, `experimentalStaticHeaders` can be used to add the CSP headers to the `Response` object instead of creating a `<meta>` element:
...
Specifies an alternate host for loading prerendered [custom error pages](/docs/basics/astro-pages/#custom-404-error-page).
...
First, [performing a build](/docs/guides/deploy/#building-your-site-locally). Depending on which `mode` selected (see above) follow the appropriate steps below:
...
Astro uses the local filesystem for session storage when using the Node adapter. If you would prefer to use a different session storage driver, you can specify it in your Astro config. See [the `session` configuration reference](/docs/reference/configuration-reference/#sessiondriver) for more details.

## Environment variables

When using the [`astro:env`](/docs/guides/environment-variables/#type-safe-environment-variables) secrets or `process.env` at runtime, neither Astro nor the adapter loads environment variables for you. 
