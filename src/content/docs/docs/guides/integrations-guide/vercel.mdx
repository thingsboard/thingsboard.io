...
This adapter allows Astro to deploy your [on-demand rendered routes and features](/docs/guides/on-demand-rendering/) to [Vercel](https://www.vercel.com/), including [server islands](/docs/guides/server-islands/), [actions](/docs/guides/actions/), and [sessions](/docs/guides/sessions/).
...
Learn how to deploy your Astro site in our [Vercel deployment guide](/docs/guides/deploy/vercel/).
...
<ReadMore>Find out more about [deploying your project to Vercel](/docs/guides/deploy/vercel/).</ReadMore>
...
The `domains` and `remotePatterns` properties will automatically be filled using [the Astro corresponding `image` settings](/docs/reference/configuration-reference/#image-options).
...
When enabled, an [Image Service](/docs/reference/image-service-reference/) powered by the Vercel Image Optimization API will be automatically configured and used in production. In development, the image service specified by [`devImageService`](#devimageservice) will be used instead.
...
Note that ISR function requests do not include search params, similar to [requests](/docs/reference/api-reference/#request) in static mode.
...
When using sessions on Vercel, you need to [configure a driver](/docs/reference/configuration-reference/#sessiondriver) for session storage. You can install a storage provider from [the Vercel marketplace](https://vercel.com/marketplace?category=storage).
...
For example, when [experimental Content Security Policy](/docs/reference/experimental-flags/csp/) is enabled, `experimentalStaticHeaders` can be used to add the CSP `headers` to your Vercel configuration, instead of creating a `<meta>` element:
