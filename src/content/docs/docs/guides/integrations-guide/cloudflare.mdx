...
This adapter allows Astro to deploy your [on-demand rendered routes and features](/docs/guides/on-demand-rendering/) to [Cloudflare](https://www.cloudflare.com/), including [server islands](/docs/guides/server-islands/), [actions](/docs/guides/actions/), and [sessions](/docs/guides/sessions/).

If you're using Astro as a static site builder, you don't need an adapter.

Learn how to deploy your Astro site in our [Cloudflare deployment guide](/docs/guides/deploy/cloudflare/).
...
* **`passthrough`:** Uses the existing [`noop`](/docs/guides/images/#configure-no-op-passthrough-service) service.
...
* **`custom`:** Always uses the image service configured in [Image Options](/docs/reference/configuration-reference/#image-options). **This option will not check to see whether the configured image service works in Cloudflare's `workerd` runtime.**
...
Cloudflare environment variables and secrets are compatible with the [`astro:env` API](/docs/guides/environment-variables/#type-safe-environment-variables).
...
You can type the `runtime` object by [extending global types](/docs/guides/typescript/#extending-global-types) using `Runtime`:
