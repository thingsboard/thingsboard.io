...
2. Choose a host that supports **continuous deployment** (e.g. [Netlify](/docs/guides/deploy/netlify/) or [Vercel](/docs/guides/deploy/vercel/)) and import your Git repository as a new site/project.
...
By default, the build output will be placed at `dist/`. This location can be changed using the [`outDir` configuration option](/docs/reference/configuration-reference/#outdir).

## Adding an Adapter for on-demand rendering

:::note
Before deploying your Astro site with [on-demand rendering](/docs/guides/on-demand-rendering/) enabled, make sure you have:

- Installed the [appropriate adapter](/docs/guides/on-demand-rendering/) to your project dependencies (either manually, or using the adapter's `astro add` command, e.g. `npx astro add netlify`).
- [Added the adapter](/docs/reference/configuration-reference/#integrations) to your `astro.config.mjs` file's import and default export when installing manually. (The `astro add` command will take care of this step for you!)
:::
