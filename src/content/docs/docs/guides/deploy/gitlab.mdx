...
1. Set up [`site`](/docs/reference/configuration-reference/#site) and [`base`](/docs/reference/configuration-reference/#base) options in `astro.config.mjs`.
    ...
    See more about [configuring a `base` value](/docs/reference/configuration-reference/#base)
    :::
...
3. Set `outDir: 'public'` in `astro.config.mjs`. This setting instructs Astro to put the static build output in a folder called `public`, which is the folder required by GitLab Pages for exposed files.
   ...
   If you were using the [`public/` directory](/docs/basics/project-structure/#public) as a source of static files in your Astro project, rename it and use that new folder name in `astro.config.mjs` for the value of `publicDir`.
