...
<ReadMore>See our [directives reference](/docs/reference/directives-reference/#classlist) page to learn more about `class:list`.</ReadMore>
...
<ReadMore>See our [directives reference](/docs/reference/directives-reference/#definevars) page to learn more about `define:vars`.</ReadMore>
...
<ReadMore>Read more about using [static assets](/docs/guides/imports/) located in `public/` or `src/`.</ReadMore>
...
You can import stylesheets in your Astro component frontmatter using ESM import syntax. CSS imports work like [any other ESM import in an Astro component](/docs/basics/astro-components/#the-component-script), which should be referenced as **relative to the component** and must be written at the **top** of your component script, with any other imports.
...
:::note
This is a [Vite-specific setting](https://vite.dev/config/ssr-options.html#ssr-noexternal) that does _not_ relate to (or require) [Astro SSR](/docs/guides/on-demand-rendering/).
:::
...
Depending on your chosen value for [`scopedStyleStrategy`](/docs/reference/configuration-reference/#scopedstylestrategy), scoped styles may or may not increase the [CLASS column specificity](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_cascade/Specificity#class_column).
...
:::tip
A common pattern in Astro is to import global CSS inside a [Layout component](/docs/basics/layouts/). Be sure to import the Layout component before other imports so that it has the lowest precedence.
:::
...
Any Astro styling methods are available to a [Markdown layout component](/docs/basics/layouts/#markdown-layouts), but different methods will have different styling effects on your page.
...
You can also add CSS integrations including [Tailwind](/docs/recipes/tailwind-rendered-markdown/). If you are using Tailwind, the [typography plugin](https://tailwindcss.com/docs/typography-plugin) can be useful for styling Markdown.
