...
Need to upgrade an older project to v1? See our [older migration guide](/docs/guides/upgrade-to/v1/).
...
2. Check your [deployment environment's](/docs/guides/deploy/) own documentation to verify that they support Node 16. 
...
Astro v2.0 now includes the Collections API for organizing your Markdown and MDX files into [content collections](/docs/guides/content-collections/). This API reserves `src/content/` as a special folder.

#### What should I do?

Rename an existing `src/content/` folder to avoid conflicts. This folder, if it exists, can now only be used for [content collections](/docs/guides/content-collections/).
...
You can control this location with the [new `build.assets` configuration option](/docs/reference/configuration-reference/#buildassets).
...
- **[`mdx.extendMarkdownConfig`](/docs/guides/integrations-guide/mdx/#extendmarkdownconfig)** to inherit all or none of your Markdown configuration
...
Astro v2.0 allows you to now individually set [every available Markdown configuration option](/docs/reference/configuration-reference/#markdown-options) (except `drafts`) separately in your MDX integration configuration.
...
Astro v2.0 requires a `svelte.config.js` file in your project if you are using [the `@astrojs/svelte` integration](/docs/guides/integrations-guide/svelte/). This is needed to provide IDE autocompletion.
...
If you were using this feature in v1.x, we recommend [using the MDX integration](/docs/guides/integrations-guide/mdx/) which allows you to combine components and JSX expressions with Markdown syntax.
...
Use `Astro.glob()` to fetch Markdown files, or convert to the [Content Collections](/docs/guides/content-collections/) feature.
...
- [Content collections](/docs/guides/content-collections/) as a way to manage your Markdown and MDX files with type-safety.
- [Prerendering individual pages to static HTML](/docs/guides/on-demand-rendering/) when using SSR to improve speed and cacheability.
