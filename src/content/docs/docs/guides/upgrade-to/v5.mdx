...
Need to upgrade an older project to v4 first? See our [older migration guide](/docs/guides/upgrade-to/v4/).
...
You can also [upgrade your Astro integrations manually](/docs/guides/integrations-guide/#manual-upgrading) if needed, and you may also need to upgrade other dependencies in your project.
...
If you are using an MDX server renderer with the experimental [Astro Container API](/docs/reference/container-reference/) you must update the import to reflect the new location:
...
<ReadMore>Learn more about [using MDX in your project](/docs/guides/integrations-guide/mdx/).</ReadMore>
...
In Astro 4.x, content collections were defined, queried, and rendered using [the Content Collections API first introduced in Astro v2.0](https://astro.build/blog/introducing-content-collections/). All collection entries were local files within the reserved `src/content/` folder. Additionally, Astro's [file name convention to exclude building individual pages](/docs/guides/routing/#excluding-pages) was built in to the Content Collections API.
...
<ReadMore>Learn more about the updated [content collections](/docs/guides/content-collections/).</ReadMore>
...
If you are not yet ready to update your existing collections, you can enable the [`legacy.collections`](/docs/reference/legacy-flags/) flag and your existing collections will continue to function as before.
...
Replace all use of `Astro.glob()` with `import.meta.glob()`. Note that `import.meta.glob()` no longer returns a `Promise`, so you may have to update your code accordingly. You should not require any updates to your [glob patterns](/docs/guides/imports/#glob-patterns).
...
Where appropriate, consider using [content collections](/docs/guides/content-collections/) to organize your content, which has its own newer, more performant querying functions.

You may also wish to consider using glob packages from NPM, such as [`fast-glob`](https://www.npmjs.com/package/fast-glob).

<ReadMore>Learn more about [importing files with `import.meta.glob`](/docs/guides/imports/#importmetaglob).</ReadMore>
...
<ReadMore>Learn more about [the Adapter API](/docs/reference/adapter-reference/) for building adapter integrations.</ReadMore>
...
<ReadMore>Learn more about [the Integration API `astro:routes:resolved` hook](/docs/reference/integrations-reference/#astroroutesresolved) for building integrations.</ReadMore>
...
<ReadMore>Learn more about [Astro's official integrations](/docs/guides/integrations-guide/).</ReadMore>
...
<ReadMore>Learn more about [on-demand rendering in Astro](/docs/guides/on-demand-rendering/).</ReadMore>
...
If your adapter does not support Astro's built-in Sharp image optimization, you can [configure a no-op image service](/docs/guides/images/#configure-no-op-passthrough-service) to allow you to use the `<Image />` and `<Picture />` components.
...
<ReadMore>Read more about [configuring your default image service](/docs/guides/images/#default-image-service).</ReadMore>
...
<ReadMore>Read more about the [route priority order rules](/docs/guides/routing/#route-priority-order).</ReadMore>
...
<ReadMore>Read more about [using `script` tags in Astro](/docs/guides/client-side-scripts/).</ReadMore>
...
<ReadMore>Read more about [view transitions and client-side routing in Astro](/docs/guides/view-transitions/).</ReadMore>
...
<ReadMore>Read more about [TypeScript configuration in Astro](/docs/guides/typescript/#setup).</ReadMore>
...
To address the "confirm form resubmission?" dialog on refresh, or to preserve action results across sessions, you can now [customize action result handling from middleware](/docs/guides/actions/#advanced-persist-action-results-with-a-session).

We recommend using a session storage provider [as described in our Netlify Blob example](/docs/guides/actions/#advanced-persist-action-results-with-a-session). However, if you prefer the cookie forwarding behavior from 4.X and accept the 4 KB size limit, you can implement the pattern as shown in this sample snippet:
...
<ReadMore>Read more about the [`compiledContent()` function](/docs/guides/markdown-content/#importing-markdown) for returning compiled Markdown.</ReadMore>
...
<ReadMore>Read more about [the `astro:content` API](/docs/reference/modules/astro-content/).</ReadMore>
...
<ReadMore>Read more about [syntax highlighting in Astro](/docs/guides/syntax-highlighting/).</ReadMore>
...
<ReadMore>Read more about [Markdown layouts](/docs/basics/layouts/#markdown-layouts).</ReadMore>
...
<ReadMore>Read more about [using Markdown plugins in Astro](/docs/guides/markdown-content/#markdown-plugins).</ReadMore>
...
<ReadMore>Read more about [setting an endpoint to use for image optimization](/docs/reference/configuration-reference/#imageendpoint).</ReadMore>
...
<ReadMore>Read more about [`build` configuration options in Astro](/docs/reference/configuration-reference/#build-options).</ReadMore>
...
<ReadMore>Read more about [Vite configuration settings in Astro](/docs/reference/configuration-reference/#vite).</ReadMore>
...
<ReadMore>Read more about [pagination in Astro](/docs/guides/routing/#pagination).</ReadMore>
...
<ReadMore>Read more about [using HTML attributes in Astro](/docs/reference/astro-syntax/#dynamic-attributes).</ReadMore>
...
<ReadMore>See more about [storing data in `context.locals`](/docs/guides/middleware/#storing-data-in-contextlocals).</ReadMore>
...
<ReadMore>Read more about [creating dynamic routes with `params`](/docs/guides/routing/#static-ssg-mode).</ReadMore>
...
<ReadMore>See the [API reference for `IntegrationRouteData`](/docs/reference/integrations-reference/#integrationroutedata).</ReadMore>
...
<ReadMore>See the [Adapter API reference for `renderOptions`](/docs/reference/adapter-reference/#renderoptions).</ReadMore>
...
<ReadMore>Read more about [specifying supported Astro features in an adapter](/docs/reference/adapter-reference/#astro-features).</ReadMore>
...
<ReadMore>Read more about [developing a dev toolbar app for Astro using the Dev Toolbar API](/docs/reference/dev-toolbar-app-reference/).</ReadMore>
