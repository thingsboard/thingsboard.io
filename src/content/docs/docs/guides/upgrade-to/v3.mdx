...
Need to upgrade an older project to v2? See our [older migration guide](/docs/guides/upgrade-to/v2/).
...
2. Check your [deployment environment's](/docs/guides/deploy/) own documentation to verify that they support Node 18.
...
Otherwise, delete all references to importing Astro's `<Markdown />` component and the component itself in your `.astro` files. You will need to rewrite your content as HTML directly or [import Markdown](/docs/guides/markdown-content/#importing-markdown) from a `.md` file.
...
Astro v3.0 removes these deprecated APIs entirely. The officially supported [configuration settings](/docs/reference/configuration-reference/) and modern `<style is:global>` and `<script>` syntax should be used instead.

#### What should I do?

If you are continuing to use v1.x APIs, use the new APIs for each feature instead:

- Deprecated config options: See [the 0.26 migration guide](/docs/guides/upgrade-to/v1/#new-configuration-api)
- Deprecated script/style attribute types: See [the 0.26 migration guide](/docs/guides/upgrade-to/v1/#new-default-script-behavior)
...
If you are using Web APIs in server-rendered components, you will need to either make the usage of those APIs conditional or use [the `client:only` client directive](/docs/reference/directives-reference/#clientonly).
...
In Astro v2.x, the [`class:list` directive](/docs/reference/directives-reference/#classlist) used a custom implementation inspired by [`clsx`](https://github.com/lukeed/clsx) with a few extra features like deduplication and `Set` support.
...
In Astro v2.x, [`class:list` values](/docs/reference/directives-reference/#classlist) were sent to components via [`Astro.props['class:list']`](/docs/reference/api-reference/#props).
...
In Astro v2.x, camelCase [CSS variables](/docs/guides/styling/#css-variables) passed to the `style` attribute were rendered as both camelCase (as written) and kebab-case (kept for backwards compatibility).
...
In Astro v2.x, the return value of [`getStaticPaths()`](/docs/reference/routing-reference/#getstaticpaths) was automatically flattened to allow you to return an array of arrays without errors.
...
An [error message indicating that `getStaticPath()`'s return value must be an array of objects](/docs/reference/errors/invalid-get-static-paths-entry/#what-went-wrong) will be provided if you need to update your code.

### Moved: `astro check` now requires an external package

In Astro v2.x, [`astro check`](/docs/reference/cli-reference/#astro-check) was included in Astro by default, and its dependencies were bundled in Astro. This meant a larger package whether or not you ever used `astro check`. This also prevented you from having control over the version of TypeScript and the Astro Language Server to use.
...
Astro v3.0 replaces these build config options with new [SSR adapter configuration options](/docs/guides/integrations-guide/#official-integrations) to perform the same tasks: `edgeMiddleware` and `functionPerRoute`.
...
To continue to mark some pages in your project as drafts, [migrate to content collections](/docs/guides/content-collections/) and manually filter out pages with the `draft: true` frontmatter property instead.
...
Astro v3.0 changes the [default port](/docs/reference/cli-reference/#--port-number) to `4321`. 🚀
...
In Astro v2.x, `import.meta.env.BASE_URL` appended your [`base`](/docs/reference/configuration-reference/#base) setting with a [trailingSlash](/docs/reference/configuration-reference/#trailingslash) by default. `trailingSlash: "ignore"` also appended a trailing slash.
...
In Astro v2.x, Astro only compressed your emitted HTML when [`compressHTML`](/docs/reference/configuration-reference/#compresshtml) was explicitly set to `true`. The default value was `false`.
...
In Astro v2.x, the default value of [`scopedStyleStrategy`](/docs/reference/configuration-reference/#scopedstylestrategy) was `"where"`.

Astro v3.0 introduces a new, default value: `"attribute"`. By default, styles are now applied using `data-*` attributes.

#### What should I do?

To retain your project's current [style scoping](/docs/guides/styling/#scoped-styles), update the configuration file to the previous default value:
...
In Astro v2.x, Squoosh was the [default image processing service](/docs/guides/images/#default-image-service).
...
In Astro v2.x, [HTTP request methods](/docs/guides/endpoints/#http-methods) were written using lowercase function names: `get`, `post`, `put`, `all`, and `del`.
...
In Astro v2.x, you could use [multiple JSX framework integrations](/docs/guides/integrations-guide/#official-integrations) (React, Solid, Preact) in the same project without needing to identify which files belonged to which framework.
...
To [run the Astro CLI programmatically](/docs/reference/programmatic-reference/), use the new experimental JavaScript APIs:
...
This import alias is no longer included by default with `astro:assets`. If you were using this alias with experimental assets, you must convert them to relative file paths, or [create your own import aliases](/docs/guides/imports/#aliases).
...
You must update the `src` attribute of any existing `<img>` tags (including any [images in UI framework components](/docs/guides/images/#images-in-ui-framework-components)) and you may also update other attributes that are now available to you from the imported image.
...
If you require more control over your image attributes, we recommend using the `.mdx` file format, which allows you to include Astro's `<Image />` component or a JSX `<img />` tag in addition to the Markdown syntax. Use the [MDX integration](/docs/guides/integrations-guide/mdx/) to add support for MDX to Astro.
...
1. Remove the `@astrojs/image` integration.
    ...
    You must [remove the integration](/docs/guides/integrations-guide/#removing-an-integration) by uninstalling and then removing it from your `astro.config.mjs` file.
...
3. Migrate any existing `<Image />` components.
    ...
    Remove any component attributes that are not [currently supported image asset properties](/docs/reference/modules/astro-assets/#image-).
...
The new `image` helper for content collections lets you validate the image metadata using Zod. Learn more about [how to use images in content collections](/docs/guides/images/#images-in-content-collections)
