...
You can now use the new [`Astro.url`](/docs/reference/api-reference/#url) helper to construct your own canonical URL from the current page/request URL. 
...
Astro no longer supports components or JSX expressions in Markdown pages by default. For long-term support you should migrate to the [`@astrojs/mdx`](/docs/guides/integrations-guide/mdx/) integration.
...
<Steps>
1. Install the [`@astrojs/mdx`](/docs/guides/integrations-guide/mdx/) integration.
...
RSS feeds should now be generated using the `@astrojs/rss` package, as described in our [RSS guide](/docs/recipes/rss/).
...
If you run Astro with legacy configuration, you will see a warning with instructions on how to update. See our updated [Configuration Reference](/docs/reference/configuration-reference/) for more information on upgrading.
...
See how to use [client-side scripts](/docs/guides/client-side-scripts/) in Astro for full details.
...
The `renderers` config has been replaced by a new, official integration system! This unlocks some really exciting new features for Astro. You can read our [Using Integrations](/docs/guides/integrations-guide/) guide for more details on how to use this new system.
...
Read our [step-by-step walkthrough](/docs/guides/integrations-guide/) to learn how to add a new Astro integration for the framework(s) that you currently use.
...
For a deeper walkthrough, read our [step-by-step guide](/docs/guides/integrations-guide/) to learn how to replace existing renderers with a new Astro framework integration.
...
If you see a `"Cannot find package 'react'"` (or similar) warning when you start up Astro, that means that you need to install that package into your project. See our [note on peer dependencies](/docs/guides/troubleshooting/#cannot-find-package-x) in the troubleshooting guide for more information.
