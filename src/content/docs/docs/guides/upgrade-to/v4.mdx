...
Need to upgrade an older project to v3? See our [older migration guide](/docs/guides/upgrade-to/v3/).
...
In Astro v3.0, `build.split` and `build.excludeMiddleware` build config options were deprecated and replaced with [adapter configuration options](/docs/reference/adapter-reference/#adapter-features) to perform the same tasks.
...
Please see the v3 migration guide to [update these deprecated middleware properties](/docs/guides/upgrade-to/v3/#deprecated-buildexcludemiddleware-and-buildsplit) with adapter configurations.
...
Update all occurrences to [`Astro.params`](/docs/reference/api-reference/#params), which is the supported replacement.
...
If you are using the unsupported method for generating RSS feeds, you must now use the [`@astrojs/rss` integration](/docs/recipes/rss/) for a complete RSS setup.
...
Please see the v3 migration guide [for guidance using uppercase HTTP request methods](/docs/guides/upgrade-to/v3/#changed-http-request-methods-case).
...
When using the Astro preview server, all of your static asset imports and URLs from the public directory must have [the base value](/docs/reference/configuration-reference/#base) prefixed to the path.
