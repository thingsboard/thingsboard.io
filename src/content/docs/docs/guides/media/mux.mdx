...
- An existing Astro project. Some features may additionally require an adapter installed for [on-demand server rendering](/docs/guides/on-demand-rendering/).
...
<ReadMore>Read more about using [environment variables](/docs/guides/environment-variables/) in your Astro project, including creating a [type-safe schema](/docs/guides/environment-variables/#type-safe-environment-variables) for your Mux credentials.</ReadMore>
...
You can listen for these events and handle them in your Astro component with [client-side scripts](/docs/guides/client-side-scripts/). A `MuxUploaderElement` type is also available.
