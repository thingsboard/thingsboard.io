...
You can add the [`is:inline`](/docs/reference/directives-reference/#isinline) directive to intentionally opt out of processing for a script.
...
In Astro components, the code in [the frontmatter](/docs/basics/astro-components/#the-component-script) (between the `---` fences) runs on the server and is not available in the browser.
...
Elements rendered by a UI framework may not be available yet when a `<script>` tag executes. If your script also needs to handle [UI framework components](/docs/guides/framework-components/), using a custom element is recommended.
