...
There is also no native video support in Astro, and we recommend choosing a [hosted video service](/docs/guides/media/) to handle the demands of optimizing and streaming video content.

<ReadMore>See the full API reference for the [`<Image />`](/docs/reference/modules/astro-assets/#image-) and [`<Picture />`](/docs/reference/modules/astro-assets/#picture-) components.</ReadMore>
...
You can also choose to store your images remotely, in a [content management system (CMS)](/docs/guides/cms/) or [digital asset management (DAM)](/docs/guides/media/) platform. Astro can fetch your data remotely using APIs or display images from their full URL path.
...
Astro's templating language allows you to render optimized images with the Astro [`<Image />`](/docs/reference/modules/astro-assets/#image-) component and generate multiple sizes and formats with the Astro [`<Picture />`](/docs/reference/modules/astro-assets/#picture-) component. Both components also accept [responsive image properties](#responsive-image-behavior) for resizing based on container size and responding to device screen size and resolution.
...
<ReadMore>See the full API reference for the [`<Image />`](/docs/reference/modules/astro-assets/#image-) and [`<Picture />`](/docs/reference/modules/astro-assets/#picture-) components including required and optional properties.</ReadMore>
...
Use standard Markdown `![alt](src)` syntax in your `.md` files. Your local images stored in `src/` and remote images will be processed and optimized. When you [configure responsive images globally](/docs/reference/configuration-reference/#imagelayout), these images will also be [responsive](#responsive-image-behavior).
...
![A starry night sky.](/docs/images/stars.png)
...
The `<Image />` and `<Picture />` components are unavailable in `.md` files. If you require more control over your image attributes, we recommend using [Astro's MDX integration](/docs/guides/integrations-guide/mdx/) to add support for `.mdx` file format. MDX allows additional [image options available in MDX](#images-in-mdx-files), including combining components with Markdown syntax.
...
Astro components (e.g. `<Image />`, `<Picture />`, SVG components) are unavailable inside UI framework components because [a client island must contain only valid code for its own framework](/docs/guides/framework-components/#can-i-use-astro-components-inside-my-framework-components).

But, you can pass the static content generated by these components to a framework component inside a `.astro` file [as children](/docs/guides/framework-components/#passing-children-to-framework-components) or using a [named `<slot/>`](/docs/guides/framework-components/#can-i-use-astro-components-inside-my-framework-components):
...
The `<Image />` component accepts [several component properties](/docs/reference/modules/astro-assets/#image-) as well as any attributes accepted by the HTML `<img>` tag.
...
<ReadMore>See details about [the `<Picture />` component properties](/docs/reference/modules/astro-assets/#picture-) in the `astro:assets` reference.</ReadMore>
...
With [responsive image properties](/docs/reference/modules/astro-assets/#responsive-image-properties) applied to the `<Image />` or `<Picture />` components, Astro will automatically generate the required `srcset` and `sizes` values for your images, and apply the necessary [styles to ensure they resize correctly](#responsive-image-styles). 

When this responsive behavior is [configured globally with `image.layout`](/docs/reference/configuration-reference/#imagelayout), it will apply to all image components and also to any local and remote images using [the Markdown `![]()` syntax](/docs/guides/images/#images-in-markdown-files).
...
Setting [`image.responsiveStyles: true`](/docs/reference/configuration-reference/#imageresponsivestyles) applies a small number of global styles to ensure that your images resize correctly. In most cases, you will want to enable these as a default; your images will not be responsive without additional styles.
...
Reference the default import of any local `.svg` file. Since this import is treated as an Astro component, you must use the same conventions (e.g. capitalization) as when [using dynamic tags](/docs/reference/astro-syntax/#dynamic-tags).
...
The [Astro template syntax](/docs/reference/astro-syntax/) also supports writing an `<img>` tag directly, with full control over its final output. These images will not be processed and optimized. It accepts all HTML `<img>` tag properties, and the only required property is `src`. However, it is strongly recommended to include [the `alt` property for accessibility](#alt-text).

### images in `src/`

Local images must be imported from the relative path from the existing `.astro` file, or you can configure and use an [import alias](/docs/guides/imports/#aliases). Then, you can access the image's `src` and other properties to use in the `<img>` tag.

Imported image assets match the [`ImageMetadata` type](/docs/reference/modules/astro-assets/#imagemetadata-1) and have the following signature:
...
[Sharp](https://github.com/lovell/sharp) is the default image service used for `astro:assets`. You can further configure the image service using the [`image.service`](/docs/reference/configuration-reference/#imageservice) option.
...
The default cache directory is `./node_modules/.astro`, however this can be changed using the [`cacheDir`](/docs/reference/configuration-reference/#cachedir) configuration setting.
