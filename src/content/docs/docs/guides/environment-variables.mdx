...
Astro also provides a way to [use and organize your environment variables with type safety](#type-safe-environment-variables). It is available for use inside the Astro context (e.g. Astro components, routes and endpoints, UI framework components, middleware), and managed with [a schema in your Astro configuration](/docs/reference/configuration-reference/#env).
...
To achieve this, you can create an `env.d.ts` in `src/` to [extend the global types](/docs/guides/typescript/#extending-global-types) and configure `ImportMetaEnv` like this:
...
- `import.meta.env.BASE_URL`: The base URL your site is being served from. This is determined by the [`base` config option](/docs/reference/configuration-reference/#base).
- `import.meta.env.SITE`: This is set to [the `site` option](/docs/reference/configuration-reference/#site) specified in your project's `astro.config`.
- `import.meta.env.ASSETS_PREFIX`: The prefix for Astro-generated asset links if the [`build.assetsPrefix` config option](/docs/reference/configuration-reference/#buildassetsprefix) is set. This can be used to create asset links not handled by Astro.
...
The `astro dev` and `astro build` commands default to `"development"` and `"production"` modes, respectively. You can run these commands with the [`--mode` flag](/docs/reference/cli-reference/#--mode-string) to pass a different value for `mode` and load the matching `.env` file.
...
<ReadMore>Developing an adapter? See how to [make an adapter compatible with `astro:env`](/docs/reference/adapter-reference/#envgetsecret).</ReadMore>
...
- **Secret server variables**: These variables are not part of your final bundle and can be accessed on the server through the `astro:env/server` module:
   ...
   You can also enable validating secrets on start by [configuring `validateSecrets: true`](/docs/reference/configuration-reference/#envvalidatesecrets).
...
<ReadMore>For a complete list of validation fields, see the [`envField` API reference](/docs/reference/configuration-reference/#envschema).</ReadMore>
...
<ReadMore>Learn more in [the API reference](/docs/reference/modules/astro-env/#getsecret).</ReadMore>
