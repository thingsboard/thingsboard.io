...
Build your Astro website without sacrificing your favorite component framework. Create Astro [islands](/docs/concepts/islands/) with the UI frameworks of your choice.
...
See the [Integrations Guide](/docs/guides/integrations-guide/) for more details on installing and configuring Astro integrations.
...
A framework component can be made interactive (hydrated) using a [`client:*` directive](/docs/reference/directives-reference/#client-directives). These are component attributes that determine when your component's JavaScript should be sent to the browser.
...
<ReadMore>See our [directives reference](/docs/reference/directives-reference/#client-directives) page for a full description of these hydration directives, and their usage.</ReadMore>
...
Astro will recognize and render your component based on its file extension. To distinguish between frameworks that use the same file extension, [additional configuration when rendering multiple JSX frameworks](/docs/guides/integrations-guide/react/#combining-multiple-jsx-frameworks) (e.g. React and Preact) is required.
...
Props that are passed to interactive framework components [using a `client:*` directive](/docs/reference/directives-reference/#client-directives) must be [serialized](https://developer.mozilla.org/en-US/docs/Glossary/Serialization): translated into a format suitable for transfer over a network, or storage. However, Astro does not serialize every type of data structure. Therefore, there are some limitations on what can be passed as props to hydrated components.
...
Additionally, you can use [Named Slots](/docs/basics/astro-components/#named-slots) to group specific children together. 
...
You can, however, use [the Astro `<slot />` pattern](/docs/basics/astro-components/#slots) to pass static content generated by Astro components as children to your framework components **inside an `.astro` component**. 
...
<ReadMore>Learn more about [client-side `<script>` tags in Astro components](/docs/guides/client-side-scripts/)</ReadMore>
