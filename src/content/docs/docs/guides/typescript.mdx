...
Astro ships with built-in support for [TypeScript](https://www.typescriptlang.org/). You can import `.ts` and `.tsx` files in your Astro project, write TypeScript code directly inside your [Astro component](/docs/basics/astro-components/#the-component-script), and even use an [`astro.config.ts`](/docs/guides/configuring-astro/#the-astro-config-file) file for your Astro configuration if you like.
...
You don't need to write TypeScript code in your Astro projects to benefit from it. Astro always treats your component code as TypeScript, and the [Astro VS Code Extension](/docs/editor-setup/) will infer as much as it can to provide autocompletion, hints, and errors in your editor.
...
This plugin runs only in the editor. When running `tsc` in the terminal, `.astro` files are ignored entirely. Instead, you can use [the `astro check` CLI command](/docs/reference/cli-reference/#astro-check) to check both `.astro` and `.ts` files.
...
Astro supports import aliases that you define in your `tsconfig.json` `paths` configuration. [Read our imports guide](/docs/guides/imports/#aliases) to learn more.
...
Astro supports typing your component props via TypeScript. To enable, add a TypeScript `Props` interface to your component frontmatter. An `export` statement may be used, but is not necessary. The [Astro VS Code Extension](/docs/editor-setup/) will automatically look for the `Props` interface and give you proper TS support when you use that component inside another template.
...
Astro includes helpers for working with the types returned by your [`getStaticPaths()`](/docs/reference/routing-reference/#getstaticpaths) function for dynamic routes.

You can get the type of [`Astro.params`](/docs/reference/api-reference/#params) with `InferGetStaticParamsType` and the type of [`Astro.props`](/docs/reference/api-reference/#props) with `InferGetStaticPropsType` or you can use `GetStaticPaths` to infer both at once:
...
<ReadMore>Read more about [`.ts` file imports](/docs/guides/imports/#typescript) in Astro.</ReadMore>
