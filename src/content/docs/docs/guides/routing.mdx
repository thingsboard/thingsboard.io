...
`.astro` [page components](/docs/basics/astro-pages/) as well as Markdown and MDX Files (`.md`, `.mdx`) within the `src/pages/` directory **automatically become pages on your website**. Each page’s route corresponds to its path and filename within the `src/pages/` directory.
...
:::tip
There is no separate "routing config" to maintain in an Astro project! When you add a file to the `src/pages/` directory, a new route is automatically created for you. In static builds, you can customize the file output format using the [`build.format`](/docs/reference/configuration-reference/#buildformat) configuration option.
:::
...
<ReadMore>Learn more about [`getStaticPaths()`](/docs/reference/routing-reference/#getstaticpaths).</ReadMore>
...
In the following example, a rest parameter (`[...slug]`) and the [`props`](/docs/reference/routing-reference/#data-passing-with-props) feature of `getStaticPaths()` generate pages for slugs of different depths.
...
You can specify a mapping of permanent redirects in your Astro config with the [`redirects`](/docs/reference/configuration-reference/#redirects) value.
...
Because Astro uses [HTML streaming](/docs/guides/on-demand-rendering/#html-streaming) in on-demand rendering, redirects must be done at the page level, not inside child components.

## Rewrites

<p><Since v="4.13.0" /></p>

A rewrite allows you to serve a different route without redirecting the browser to a different page. The browser will show the original address in the URL bar, but will instead display the content of the URL provided to [`Astro.rewrite()`](/docs/reference/api-reference/#rewrite).
...
<ReadMore>See the [`Astro.rewrite()` API reference](/docs/reference/api-reference/#rewrite) for more information.</ReadMore>
...
- `_server_islands/`: Serves the dynamic components deferred into a [server island](/docs/guides/server-islands/).
- `_actions/`: Serves any defined [actions](/docs/guides/actions/).
...
<ReadMore>Learn more about [the pagination `page` prop](/docs/reference/routing-reference/#the-pagination-page-prop).</ReadMore>
