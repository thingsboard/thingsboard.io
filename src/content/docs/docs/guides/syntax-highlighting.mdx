...
You can [add a Prism stylesheet and switch to Prism's highlighting](#add-a-prism-stylesheet), or disable Astro's syntax highlighting entirely, with the [`markdown.syntaxHighlight`](/docs/reference/configuration-reference/#markdownsyntaxhighlight) configuration option.

<ReadMore>See the full [`markdown.shikiConfig` reference](/docs/reference/configuration-reference/#markdownshikiconfig) for the complete set of Markdown syntax highlighting options available when using Shiki.</ReadMore>
...
```
<ReadMore>See the full [Shiki config reference](/docs/reference/configuration-reference/#markdownshikiconfig) for the complete set of Markdown code block options.</ReadMore>
...
You can reference the `Props` of these components using the [`ComponentProps` type](/docs/guides/typescript/#componentprops-type) utility.
...
2. Add this stylesheet to [your project's `public/` directory](/docs/basics/project-structure/#public).

3. Load this into your page's `<head>` in a [layout component](/docs/basics/layouts/) via a `<link>` tag. (See [Prism basic usage](https://prismjs.com/#basic-usage).)
</Steps>
