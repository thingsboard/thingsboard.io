...
Sessions require a storage driver to store the session data. The [Node](/docs/guides/integrations-guide/node/#sessions), [Cloudflare](/docs/guides/integrations-guide/cloudflare/#sessions), and [Netlify](/docs/guides/integrations-guide/netlify/#sessions) adapters automatically configure a default driver for you, but other adapters currently require you to [specify a driver manually](/docs/reference/configuration-reference/#sessiondriver). 
...
<ReadMore>
  See [the `session` configuration option](/docs/reference/configuration-reference/#session-options) for more details on setting a storage driver, and other configurable options.
</ReadMore>


## Interacting with session data

The [`session` object](/docs/reference/api-reference/#session) allows you to interact with the stored user state (e.g. adding items to a shopping cart) and the session ID (e.g. deleting the session ID cookie when logging out). The object is accessible as `Astro.session` in your Astro components and pages and as `context.session` object in API endpoints, middleware, and actions.

The session is generated automatically when it is first used and can be regenerated at any time with [`session.regenerate()`](/docs/reference/api-reference/#regenerate) or destroyed with [`session.destroy()`](/docs/reference/api-reference/#destroy).

For many use cases, you will only need to use [`session.get()`](/docs/reference/api-reference/#get) and [`session.set()`](/docs/reference/api-reference/#set). 

<ReadMore>
See [the Sessions API reference](/docs/reference/api-reference/#session) for more details.
</ReadMore>
...
You can optionally [define TypeScript types](/docs/guides/typescript/#extending-global-types) for your session data by creating a `src/env.d.ts` file and adding a declaration for the `App.SessionData` type:
