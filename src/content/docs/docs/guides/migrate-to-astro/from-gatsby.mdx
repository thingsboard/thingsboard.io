...
- The [syntax of `.astro` files is similar to JSX](/docs/reference/astro-syntax/#jsx-like-expressions). Writing Astro should feel familiar.

- Astro has built-in support for [Markdown](/docs/guides/markdown-content/) and an integration for using MDX files. Also, you can configure and continue to use many of your existing Markdown plugins.

- Astro also has an [official integration for using React components](/docs/guides/integrations-guide/react/). Note that in Astro, React files **must** have a `.jsx` or `.tsx` extension.

- Astro has support for [installing NPM packages](/docs/guides/imports/#npm-packages), including React libraries. Many of your existing dependencies will work in Astro.

- Like Gatsby, Astro projects can be SSG or [SSR with page-level prerendering](/docs/guides/on-demand-rendering/).
...
- [Astro components](/docs/basics/astro-components/) are not written as exported functions that return page templating. Instead, you'll split your code into a "code fence" for your JavaScript and a body exclusively for the HTML you generate.

- [Local file data](/docs/guides/imports/): Gatsby uses GraphQL to retrieve data from your project files. Astro uses ESM imports and top-level await functions (e.g. [`import.meta.glob()`](/docs/guides/imports/#importmetaglob), [`getCollection()`](/docs/guides/content-collections/#querying-collections)) to import data from your project files. You can manually add GraphQL to your Astro project but it is not included by default.
...
You can pass a `--template` argument to the `create astro` command to start a new Astro project with one of our official starters (e.g. `docs`, `blog`, `portfolio`). Or, you can [start a new project from any existing Astro repository on GitHub](/docs/install-and-setup/#install-from-the-cli-wizard).
...
You may find it useful to install some of [Astro's optional integrations](/docs/guides/integrations-guide/) to use while converting your Gatsby project to Astro:
...
Following [Astro's project structure](/docs/basics/project-structure/):
...
3. **Copy or Move** Gatsby's other files and folders (e.g. `components`, `pages`, etc.) as needed into your Astro `src/` folder as you rebuild your site, following [Astro's project structure](/docs/basics/project-structure/).
...
3. Move any necessary JavaScript, including import statements, into a ["code fence" (`---`)](/docs/basics/astro-components/#the-component-script). Note: JavaScript to [conditionally render content](/docs/reference/astro-syntax/#dynamic-html) is often written inside the HTML template directly in Astro.

4. Use [`Astro.props`](/docs/reference/api-reference/#props) to access any additional props that were previously passed to your Gatsby function.

5. Decide whether any imported components also need to be converted to Astro. With the official React integration installed, you can [use existing React components in your Astro files](/docs/guides/framework-components/). But, you may want to convert them to `.astro` components, especially if they do not need to be interactive!

6. Remove any GraphQL queries. Instead, use import and [`import.meta.glob()`](/docs/guides/imports/#importmetaglob) statements to query your local files.
...
You may find it helpful to start by converting your Gatsby layouts and templates into [Astro layout components](/docs/basics/layouts/).

Each Astro page explicitly requires `<html>`, `<head>`, and `<body>` tags to be present, so it is common to reuse a layout file across pages. Astro uses a [`<slot />`](/docs/basics/astro-components/#slots) instead of React's `{children}` prop for page content, with no import statement required. Your Gatsby `layout.js` and templates will not include these.
...
In Gatsby, your [pages and posts](/docs/basics/astro-pages/) may exist in `src/pages/` or outside of `src` in another folder, like `content`. In Astro, all your page content must live within `src/` unless you are using [content collections](/docs/guides/content-collections/).
...
These [`.astro` pages](/docs/basics/astro-pages/) must be located within `src/pages/` and will have page routes generated automatically based on their file path.

#### Markdown and MDX pages

Astro has built-in support for Markdown and an optional integration for MDX files. Your existing [Markdown and MDX files](/docs/guides/markdown-content/) can be reused but may require some adjustments to their frontmatter, such as adding [Astro's special `layout` frontmatter property](/docs/basics/layouts/#markdown-layouts). They can also be placed within `src/pages/` to take advantage of automatic file-based routing.

Alternatively, you can use [content collections](/docs/guides/content-collections/) in Astro to store and manage your content. You will retrieve the content yourself and [generate those pages dynamically](/docs/guides/content-collections/#generating-routes-from-content).
...
See Astro's [testing guide](/docs/guides/testing/) for more.
...
- `gatsby-ssr.js`: If you choose to use SSR in Astro, you will [add and configure the SSR adapter](/docs/guides/on-demand-rendering/) of your choice directly in `astro.config.mjs`.

## Reference: Convert to Astro Syntax

The following are some examples of Gatsby-specific syntax that you will need to convert to Astro. See more [differences between Astro and JSX](/docs/reference/astro-syntax/#differences-between-astro-and-jsx) in the guide to writing Astro components.
...
If necessary, update any [file imports](/docs/guides/imports/) to reference relative file paths exactly. This can be done using [import aliases](/docs/guides/typescript/#import-aliases), or by writing out a relative path in full. 
...
React components that pass multiple sets of children can be migrated to an Astro component using [named slots](/docs/basics/astro-components/#named-slots). 

See more about [specific `<slot />` usage in Astro](/docs/basics/astro-components/#slots).
...
If necessary, convert any inline style objects (`style={{ fontWeight: "bold" }}`) to inline HTML style attributes (`style="font-weight:bold;"`). Or, use an [Astro `<style>` tag](/docs/guides/styling/#styling-in-astro) for scoped CSS styles.
...
Tailwind is supported after installing the [Tailwind Vite plugin](/docs/guides/styling/#tailwind). No changes to your existing Tailwind code are required!
...
See more about [Styling in Astro](/docs/guides/styling/).

### Gatsby Image Plugin to Astro

Convert Gatsby's `<StaticImage />` and `<GatsbyImage />` components to [Astro's own image integration components](/docs/guides/images/), or to a [standard HTML `<img>` / JSX `<img />`](/docs/guides/images/#images-in-ui-framework-components) tag as appropriate in your React components.
...
Astro's `<Image />` component works in `.astro` and `.mdx` files only. See a [full list of its component attributes](/docs/reference/modules/astro-assets/#image-) and note that several will differ from Gatsby's attributes.

To continue using [images in Markdown (`.md`) files](/docs/guides/images/#images-in-markdown-files) using standard Markdown syntax (`![]()`), you may need to update the link. Using the HTML `<img>` tag directly is not supported in `.md` files for local images, and must be converted to Markdown syntax.
...
You can learn more about [using images in Astro](/docs/guides/images/) in the Images Guide.

### Gatsby GraphQL to Astro

Remove all references to GraphQL queries, and instead use [`import.meta.glob()`](/docs/guides/imports/#importmetaglob) to access data from your local files. 

Or, if using content collections, query your Markdown and MDX files using [`getEntry()` and `getCollection()`](/docs/guides/content-collections/#generating-routes-from-content). 
...
5. Update `index.astro` to use this new layout and pass it the necessary `title` and `pathname` props:
    ...
    ```
    :::tip
    You can [get the current page's path using `Astro.url`](/docs/reference/api-reference/#url).
    :::
