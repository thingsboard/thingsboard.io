...
- Hugo and Astro are both modern static-site generators, ideally suited to [content-driven websites](/docs/concepts/why-astro/#content-driven) like blogs.

- Hugo and Astro both allow you to [author your content in Markdown](/docs/guides/markdown-content/). However, Hugo includes several special frontmatter properties and allows you to write frontmatter in YAML, TOML or JSON. Even though many of your existing Hugo frontmatter properties will not be "special" in Astro, you can continue to use your existing Markdown files and YAML (or TOML) frontmatter values.
...
- Hugo uses Go Templating for page templating. [Astro syntax](/docs/basics/astro-components/) is a JSX-like superset of HTML.

- Astro does not use shortcodes for dynamic content in standard Markdown files, but [Astro's MDX integration](/docs/guides/integrations-guide/mdx/) does allow you to use JSX and import components in MDX files.

- While Hugo can use "partials" for reusable layout elements, [Astro is entirely component-based](/docs/basics/astro-components/). Any `.astro` file can be a component, a layout or an entire page, and can import and render any other Astro components. Astro components can also include [other UI framework components (e.g. React, Svelte, Vue, Solid)](/docs/guides/framework-components/) as well as content or metadata from [other files in your project](/docs/guides/imports/), such as Markdown or MDX.
...
Bring your existing Markdown (or MDX, with our optional integration) files as content to [create Markdown or MDX pages](/docs/guides/markdown-content/). Astro allows YAML or TOML frontmatter in these files, so if you are using JSON frontmatter, you will need to convert it.

To continue to use dynamic content such as variables, expressions or UI components within your Markdown content, add Astro's optional MDX integration and convert your existing Markdown files to [MDX pages](/docs/guides/markdown-content/). MDX supports YAML and TOML frontmatter, so you can keep your existing frontmatter properties. But, you must replace any shortcode syntax with [MDX's own syntax](https://mdxjs.com/docs/what-is-mdx/#mdx-syntax), which allows JSX expressions and/or component imports.
