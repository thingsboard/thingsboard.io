...
Astro's [React integration](/docs/guides/integrations-guide/react/) provides support for [using React components inside Astro components](/docs/guides/framework-components/), including entire React apps like Create React App (CRA)!
...
- [content-driven](/docs/concepts/why-astro/#content-driven): Astro was designed to showcase your content and to allow you to opt-in to interactivity only as needed. An existing CRA app might be built for high client-side interactivity and may require advanced Astro techniques to include items that are more challenging to replicate using `.astro` components, such as dashboards.
...
Your app needs a [client directive](/docs/reference/directives-reference/#client-directives) for interactivity. Astro will render your React app as static HTML until you opt-in to client-side JavaScript.
...
You will replicate a similar component-based design [using Astro HTML templating components for your basic structure](/docs/basics/astro-components/) while importing and including individual React components (which may themselves be entire apps!) for islands of interactivity.
...
You may find it helpful to learn about [Astro islands](/docs/concepts/islands/) and [Astro components](/docs/basics/astro-components/) before restructuring your CRA as an Astro project.
...
4. Use [`Astro.props`](/docs/reference/api-reference/#props) to access any additional props that were previously passed to your CRA function.
...
6. Replace `useEffect()` with import statements or [`import.meta.glob()`](/docs/guides/imports/#importmetaglob) to query your local files. Use `fetch()` to fetch external data.
...
To [fetch remote data](/docs/guides/data-fetching/), use `fetch()`.
