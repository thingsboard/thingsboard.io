...
- The [syntax of `.astro` files is similar to JSX](/docs/reference/astro-syntax/#differences-between-astro-and-jsx). Writing Astro should feel familiar.
- Astro projects can also be SSG or [SSR with page-level prerendering](/docs/guides/on-demand-rendering/).
- Astro uses file-based routing, and [allows specially named pages to create dynamic routes](/docs/guides/routing/#dynamic-routes).
- Astro is [component-based](/docs/basics/astro-components/), and your markup structure will be similar before and after your migration.
- Astro has [official integrations for React, Preact, and Solid](/docs/guides/integrations-guide/react/) so you can use your existing JSX components. Note that in Astro, these files **must** have a `.jsx` or `.tsx` extension.
...
- [content-driven](/docs/concepts/why-astro/#content-driven): Astro was designed to showcase your content and to allow you to opt-in to interactivity only as needed. An existing Next.js app might be built for high client-side interactivity and may require advanced Astro techniques to include items that are more challenging to replicate using `.astro` components, such as dashboards.
...
You may find it useful to install some of [Astro's optional integrations](/docs/guides/integrations-guide/) to use while converting your Next project to Astro:
...
2. **Copy or Move** Next's other files and folders (e.g. `pages`, `styles` etc.) into Astro's `src/` folder as you rebuild your site, following [Astro's project structure](/docs/basics/project-structure/).
...
Astro has a configuration file at the root of your project called [`astro.config.mjs`](/docs/guides/configuring-astro/). This is used only for configuring your Astro project and any installed integrations, including [SSR adapters](/docs/guides/deploy/).
...
4. Use [`Astro.props`](/docs/reference/api-reference/#props) to access any additional props that were previously passed to your Next function.

5. Decide whether any imported components also need to be converted to Astro. With the official integration installed, you can [use existing React components in your Astro file](/docs/guides/framework-components/). But, you may want to convert them to `.astro` components, especially if they do not need to be interactive!

6. Replace `getStaticProps()` with import statements or [`import.meta.glob()`](/docs/guides/imports/#importmetaglob) to query your local files. Use `fetch()` to fetch external data.
...
You may find it helpful to start by converting your Next.js layouts and templates into [Astro layout components](/docs/basics/layouts/).
...
Each Astro page explicitly requires `<html>`, `<head>`, and `<body>` tags to be present, so it is common to reuse a layout file across pages. Astro uses a [`<slot />`](/docs/basics/astro-components/#slots) for page content, with no import statement required. Note the standard HTML templating, and direct access to `<head>`:  
...
In Astro, all your page content must live within `src/` unless you are using [content collections](/docs/guides/content-collections/).
...
Astro has built-in support for Markdown and an optional integration for MDX files. You can reuse any existing [Markdown and MDX files](/docs/guides/markdown-content/), but they may require some adjustments to their frontmatter, such as adding [Astro's special `layout` frontmatter property](/docs/basics/layouts/#markdown-layouts). You will no longer need to manually create pages for each Markdown-generated route. These files can be placed within `src/pages/` to take advantage of automatic file-based routing.
...
Update any [file imports](/docs/guides/imports/) to reference relative file paths exactly. This can be done using [import aliases](/docs/guides/typescript/#import-aliases), or by writing out a relative path in full. 
...
Convert any instances of `getStaticProps()` to either `import.meta.glob()` or `getCollection()`/`getEntry()` in order to access data from other files in your project source. To [fetch remote data](/docs/guides/data-fetching/), use `fetch()`.
...
See more about local files imports with [`import.meta.glob()`](/docs/guides/imports/#importmetaglob), [querying using the Collections API](/docs/guides/content-collections/#querying-collections) or [fetching remote data](/docs/guides/data-fetching/).
...
Convert any Next `<Image />` components to [Astro's own image component](/docs/guides/images/) in `.astro` or `.mdx` files, or to a [standard HTML `<img>` / JSX `<img />`](/docs/guides/images/#images-in-ui-framework-components) tag as appropriate in your React components. 

Astro's `<Image />` component works in `.astro` and `.mdx` files only. See a [full list of its component attributes](/docs/reference/modules/astro-assets/#image-) and note that several will differ from Next's attributes. 
