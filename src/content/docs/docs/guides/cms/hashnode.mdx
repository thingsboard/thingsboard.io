...
<Steps>
1. Create the page `src/pages/post/[slug].astro` to [dynamically generate a page](/docs/guides/routing/#dynamic-routes) for each post.
...
3. Create the template for each page using the properties of each `post` object. The example below shows the post title and reading time, then the full post content:
    ...
    ```
    :::note
    `<Fragment />` is a built-in Astro component which allows you to avoid an unnecessary wrapper element. This can be especially useful when fetching HTML from a CMS (e.g. Hashnode or [WordPress](/docs/guides/cms/wordpress/)).
    :::
</Steps>
