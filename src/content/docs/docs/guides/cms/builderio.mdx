...
Create the page `src/pages/posts/[slug].astro` that will [dynamically generate a page](/docs/guides/routing/#dynamic-routes) for each post.
...
- A [`getStaticPaths()`](/docs/reference/routing-reference/#getstaticpaths) function to fetch `slug` information from Builder and create a static route for each blog post.
...
:::note
The variables `builderModel` and `builderAPIpublicKey` need to be created twice, since [`getStaticPaths()` runs in its own isolated scope](/docs/reference/routing-reference/#getstaticpaths).
:::
