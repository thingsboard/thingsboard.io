...
1. **An Astro project** - If you don't have an Astro project yet, our [Installation guide](/docs/install-and-setup/) will get you up and running in no time.
...
<ReadMore>Learn more about [environment variables](/docs/guides/environment-variables/) and `.env` files in Astro.</ReadMore>
...
Create the file `src/pages/blog/[slug].astro` to [dynamically generate a page](/docs/guides/routing/#dynamic-routes) for each article.
...
In Astro's default static mode (SSG), use [`getStaticPaths()`](/docs/reference/routing-reference/#getstaticpaths) to fetch your list of articles from Strapi.
...
```
:::tip
Make sure to choose the right rendering for your content. For markdown check out our [markdown guide](/docs/guides/markdown-content/). If you are rendering html refer to [this guide](/docs/reference/directives-reference/#sethtml) for safety.
:::

#### On-demand rendering

If you've [opted into on-demand rendering with an adapter](/docs/guides/on-demand-rendering/), [generate your dynamic routes](/docs/guides/routing/#on-demand-dynamic-routes) using the following code:
...
To deploy your website, visit our [deployment guides](/docs/guides/deploy/) and follow the instructions for your preferred hosting provider.
