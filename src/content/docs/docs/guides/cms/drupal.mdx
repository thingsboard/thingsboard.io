...
<ReadMore>Read more about [using environment variables](/docs/guides/environment-variables/) and `.env` files in Astro.</ReadMore>
...
This example uses Astro's default static mode, and creates [a dynamic routing page file](/docs/guides/routing/#dynamic-routes) with the `getStaticPaths()` function. This function will be called at build time to generate the list of paths that become pages.
