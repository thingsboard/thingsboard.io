...
- A new or existing Astro project configured for [on-demand rendering](/docs/guides/on-demand-rendering/).
...
To create a page for each blog post, you will execute a new GraphQL query on a [dynamic routing](/docs/guides/routing/#on-demand-dynamic-routes) `.astro` page. This query will fetch a specific article by its slug and a new page will be created for each individual blog post.
...
To deploy your Prepr blog, visit our [deployment guides](/docs/guides/deploy/) and follow the instructions for your preferred hosting provider.
