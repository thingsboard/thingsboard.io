...
The fastest way to start is to download a sample template and adapt it. Pick the one closest to the solution you want to build, then follow the steps below to adapt and upload it:
...
2. Replace the files in `dashboards/`, `device_profiles/`, `asset_profiles/`, `rule_chains/`, and `calculated_fields/` with exports from your ThingsBoard instance. Keep named references (`entityNameFilter`, `defaultRuleChainId`) aligned with the entities declared in the next step.
3. Update the JSON arrays in `entities/` to declare every entity the solution creates — devices, assets, customers, dashboards, rule chains, and emulators.
...
5. Zip the template contents — select the files inside the folder, not the folder itself. Open the <a href="https://iot-hub.thingsboard.io/" target="_blank" rel="noopener noreferrer"><strong>IoT Hub Creator Portal</strong></a>, click **+ Add new item**, and complete the upload form with edition, version, categories, and use cases — see **[Editions](#editions)**.
</Steps>
...
- **Gallery images** — the same spec every screenshot in the marketplace follows: at least **1280 px wide** (1600×900 recommended), **16:9** — the ratio the gallery frame is built for, other ratios are letterboxed rather than cropped — **PNG** or **JPG**, up to **2 MB** per file and up to **8** per listing.
- Use **PNG** for screenshots with text or charts; **JPG** for photos.
- Always include at least 2–3 gallery images — users browse the gallery before clicking install, and any image opens full screen when clicked.
- Crop to show dashboard content, not surrounding chrome (toolbars, navigation).
- The gallery opens with `previewImageUrl` and then follows `imageUrls` in order, so list them the way you want them read.
...
<Aside type="note">
  Use only heading levels <code>##</code> through <code>######</code> in the readme content. The detail page renders its own <code>&#60;h1&#62;</code> for the item's title, so a <code>#</code> heading in your readme would produce a duplicate top-level heading.
</Aside>
...
* **🎛️ SCADA Dashboard:** Real-time visualization of water temperature, pump
  status, and valve positions.
* **🤖 Modbus Emulator:** A Docker environment simulating 14 distinct devices...

#### 🌟 Why use this template?

* **💧 Water Quality Management:** Monitor the **Water level meter** and
  **Filter pH sensor** without manual checks.
* **⚙️ Equipment Health:** Track vibration, rotation speed, and pressure to
  prevent downtime.

#### ✨ Key Features

* **Unified SCADA View:** 14 subsystems in one real-time command center.
* **Remote Management:** Switch modes or open/close valves from the dashboard.

#### 🌍 Real-World Applications

* **🏊 Public Swimming Pools** — centralize monitoring, reduce maintenance costs.
* **🏨 Hotels & Spas** — keep water temperature and clarity consistent automatically.
```
...
- Use <a href="https://semver.org/" target="_blank" rel="noopener noreferrer nofollow"><strong>semantic versioning</strong></a>: `1.0.0` for the initial release, `1.1.0` for new features, `1.0.1` for fixes.
...
- [ ] `imageUrls` contains at least two gallery images, each at least 1280 px wide and under 2 MB, no more than eight in total
