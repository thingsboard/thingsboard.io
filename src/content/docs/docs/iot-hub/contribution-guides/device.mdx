...
If your device doesn't fit any of these categories, <a href={IOT_HUB_CONTACT_US_URL} target="_blank" rel="noopener noreferrer">contact us</a> or email <a href={`mailto:${IOT_HUB_CONTACT_EMAIL}`}>{IOT_HUB_CONTACT_EMAIL}</a> to request a new hardware type.
...
- Order them so the one that best explains the device comes first — it is the slide right after the preview image
