...
| `INTEGRATION_CHIRPSTACK`           | `chirpstack-integration/` |
...
If your device doesn't fit any of these categories, contact us at <a href="mailto:iothub@thingsboard.io">iothub@thingsboard.io</a> to request a new hardware type.
...
#        - "50000:50000/udp" # Socket connector with type UDP
    ...
    environment:
      - TB_GW_HOST=${mqtt.host}
      - TB_GW_PORT=${mqtt.port}
...
Screenshots are a different thing from the images inside the ZIP described above. Those illustrate the install instructions; screenshots are the gallery shown on your device's detail page in IoT Hub, and they live on the listing rather than in the package. They are optional, but they let a user see the device and its dashboard before deciding to install.

Upload them in the **Screenshots** step of the new-item and new-version wizards, or later from the **Screenshots** tab of the version drawer. The gallery renders them in the order you arrange them, and any screenshot opens full screen when clicked.
...
- Lead with the screenshot that best explains what the device does — it is the one users see first
- Cover both sides of the package: the hardware itself and the dashboard it installs
- Use real telemetry, not an empty dashboard
...
- [ ] Each screenshot is at least 1280 px wide, PNG or JPG, and under 2 MB
- [ ] No more than 8 screenshots, arranged so the most explanatory one comes first
