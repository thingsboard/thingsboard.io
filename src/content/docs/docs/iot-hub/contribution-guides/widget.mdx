...
- **Opening paragraph** — one or two sentences on what the widget displays and its key visual behavior (e.g. color shifts, auto-scaling).
- **Who it's for** — one short paragraph describing the persona or question the widget answers, so users can immediately judge whether it fits their use case.
- **What it does** — a bullet list of notable features: layout behavior, visual indicators, optional elements, configurable appearance.
- **How to set up → Data Keys** — a table listing every telemetry or attribute key the widget reads: key name, role, type (`Timeseries` / `Attribute`), and what the value means (units, expected range).
- **How to Customize** — a bullet list of the settings users are most likely to change, each written as `**To do X** — do Y`.
- **Tips** *(optional)* — short, non-obvious advice that helps users get the most out of the widget.
...
Displays the latest temperature reading as a single scalable card, with the
icon and value color shifting automatically as the reading moves from cold
to hot — so a glance tells you the zone, not just the number.

### Who it's for

Dashboard authors who want a temperature reading to be readable at a
glance — anyone asking "is this zone running hot or cold right now?"
without needing to check units, thresholds, or a separate chart first.

### What it does

- Auto-scaling layout that fits any card size
- Icon and value color both follow the same temperature range, so they
  always agree
...
- **To hide the label, icon, or last-updated time** — turn off **Show
  label**, **Show icon**, or **Show date**.
- **To reposition the label** — change **Label position** to top or
  bottom.
- **To resize the icon** — change **Icon size**.
- **To use your own temperature bands** — edit the **Icon color ranges**
  and **Value color ranges** lists; each entry sets a `to` boundary and a
  color.
- **To switch °C/°F** — change **Units**.
- **To match your dashboard's font** — set **Value font** and **Label
  font**.

### Tips (optional)

- Keep **Icon color ranges** and **Value color ranges** using the same
  boundaries, so the icon and the number always agree on the zone.
```
