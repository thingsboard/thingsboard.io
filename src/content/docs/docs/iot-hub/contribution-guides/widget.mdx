...
3. Go to **IoT Hub → + Add new item**. A five-step wizard walks you through the rest:
    ...
    - **Screenshots** — optionally upload screenshots of the widget in use (see [Screenshots](#screenshots)).
...
Screenshots are a separate set of images from the [preview image](#preview-image) — the preview is the single card image, screenshots are the gallery on your widget's detail page. They are optional, but a widget with screenshots is much easier to evaluate: users see the widget in more than one state before they install it.
...
- Lead with the screenshot that best explains what the widget is — it is the one users see first
...
- [ ] Preview image is at least 1200 px wide and under 500 KB
