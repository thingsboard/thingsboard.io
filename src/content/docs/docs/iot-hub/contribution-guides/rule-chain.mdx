...
<Aside type="note">
Use only heading levels `##` through `######` in the readme content. The detail page renders its own `<h1>` for the item's title, so a `#` heading in your readme would produce a second, duplicate top-level heading.
</Aside>
