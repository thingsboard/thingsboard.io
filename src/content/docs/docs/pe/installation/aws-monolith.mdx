...
Where `FROM_VERSION` is the starting version. See <DocLink product={Products.PE} path="TODO">Upgrade Instructions</DocLink> for valid `fromVersion` values. You must upgrade versions one by one (e.g., 3.6.1 → 3.6.2 → 3.6.3).
