...
Now that you have a blog using Astro's [built-in file-based routing](/docs/guides/routing/#static-routes), you will update it to use a [content collection](/docs/guides/content-collections/). Content collections are a powerful way to manage groups of similar content, such as blog posts.
...
3. Create a `src/content.config.ts` file to [define a schema](/docs/guides/content-collections/#defining-the-collection-schema) for your `postsCollection`. For the existing blog tutorial code, add the following contents to the file to define all the frontmatter properties used in its blog posts:
...
2. Add the following code to [query your collection](/docs/guides/content-collections/#querying-collections) to make each blog post's slug and page content available to each page it will generate:
...
<Steps>
5. Anywhere you have a list of blog posts, like the tutorial's Blog page (`src/pages/blog.astro/`), you will need to replace `import.meta.glob()` with [`getCollection()`](/docs/reference/modules/astro-content/#getcollection) as the way to fetch content and metadata from your Markdown files.
