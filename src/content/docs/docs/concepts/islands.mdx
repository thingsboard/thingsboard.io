...
SPAs have their place, even [embedded inside an Astro page](/docs/guides/migrate-to-astro/from-create-react-app/). But, SPAs lack the native ability to selectively and strategically hydrate, making them a heavy-handed choice for most projects on the web today.
...
Even better, you can tell Astro exactly how and when to render each component. If that image carousel is really expensive to load, you can attach a special [client directive](/docs/reference/directives-reference/#client-directives) that tells Astro to only load the carousel when it becomes visible on the page. If the user never sees it, it never loads.
...
<ReadMore>Read more about [using JavaScript framework components](/docs/guides/framework-components/) in your project.</ReadMore>
...
Add the [`server:defer` directive](/docs/reference/directives-reference/#server-directives) to any Astro component on your page to turn it into its own server island:
...
<ReadMore>Read more about [using server islands](/docs/guides/server-islands/) in your project.</ReadMore>
